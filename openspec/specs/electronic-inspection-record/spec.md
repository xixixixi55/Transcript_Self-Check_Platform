...
系统 MUST 通过版本化内置适配器 `pinghang-mobile-multipath-v1` 支持当前已确认结构的平航手机多路取证离线网页报告。识别 MUST 联合使用入口、导航、版本化报告页锚点、ViewData 固定赋值前缀、页面类型和字段标签，不得只依赖目录名、案件名或样本文件编号。报告信息页 MUST 由 v1 `0_1.json` 锚点加结构/标签校验确认，案件页 MUST 由导航“案件信息”节点定位，材料页 MUST 由 `DeviceInfo` 节点定位；未选中的 ViewData 不得枚举或读取。报告内 JavaScript、工具程序和附件内容 MUST NOT 被执行；被选 `.json` 后缀 JSONP 页面只能按受限数据字面量读取。报告总容量和非核心页面数量不得阻止案件初始化；未知变体、适配器冲突、导航/材料顺序歧义和重复检材编号 MUST 阻止正式导出，不得静默回退。
...
- **THEN** 系统只读取入口、导航、报告信息页、案件信息页和导航选中的材料页
