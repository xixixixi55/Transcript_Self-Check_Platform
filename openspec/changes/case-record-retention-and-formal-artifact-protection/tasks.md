...
- T022/2.5 living sync evidence：`openspec/specs/data-model.md` 补充 cleaned tombstone 的 claim/authority/blocker 前置条件、case_drafts 原子删除、shell safe summary/identity 保留、formal rows 保留、`records_cleaned` 边界和 `CASE_RECORD_CLEANED` 不可编辑行为；明确 snapshot/source/task whitelist cleanup、物理文件删除和公共 artifact 查询仍未实现。
