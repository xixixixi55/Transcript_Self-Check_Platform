...
#### Scenario: 案件归档先确认后执行既有删除能力
- **WHEN** 用户点击案件卡片的“归档案件”操作
- **THEN** 页面弹出标题为“确认归档该案件？”的确认提示
- **AND** 已导出案件直接显示“归档案件”推荐操作；其他状态将该操作收纳到更多菜单
...
- AND 「待导出」阶段的更多菜单同时提供「打开案件」与「归档案件」；打开案件进入既有审核编辑路由，不触发统一导出，也不改变案件状态
...
- WHEN 任意状态案件通过当前阶段入口执行「归档案件」并完成确认
