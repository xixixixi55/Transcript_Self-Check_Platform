...
#### Scenario: 平航 Android 设备明确映射为手机

- **WHEN** 命中 v1 的平航材料页“数据类型”为 `Android设备`，且仅存在大小写、全半角或空白差异
- **THEN** 系统将该材料归类为 `phone` 并标记为 `confirmed_by_report`
- **AND** 手机显示策略只输出已有 IMEI1/IMEI2，不输出序列号
- **AND** 该映射不得扩展到未命中 v1 的报告或其他无法确认的类型值
