...
需求6补充：来源目录白名单校验及其浏览器开关已移除，使用统一的本机路径安全和报告结构检查；导出目标授权仍独立保留。
...
阶段一的最终检材类型只允许 `phone` 或 `tablet`。用户确认始终优先；报告“手机品牌”规范化后精确为 `iPhone`/`iPad` 时分别自动确认 `phone`/`tablet`，该 Apple 品牌事实优先于报告“设备类型=手机”。明确 `tablet` 或类型冲突继续受保护；其后只有 IMEI1 与 IMEI2 均为有效、不同的 15 位数字，系统才可推断 `phone` 并记录推断诊断；报告“设备类型=手机”仅作为最后一个自动候选。单个、非法或重复 IMEI 不得触发双 IMEI 推断。其余无法可靠判断的材料 MUST 保持待确认并由审核页面确认；每个检材在最终导出前 MUST 完成类型确认。原始 identifiers MUST 保留在 `CanonicalInspectionCase` 中，显示规则只能控制模板可见性。
...
#### Scenario: 双 IMEI 作为缺少可靠类型时的受控兜底

- **WHEN** 报告没有可靠类型来源，且 IMEI1 与 IMEI2 均为有效、不同的 15 位数字
- **THEN** 系统选择 `phone` 并以报告确认状态保留“双 IMEI 推断”诊断，审核页面不得将其描述为报告明确类型字段
- **AND** 该报告确认状态无需人工二次确认即可通过检材类型导出门控，用户仍可在审核页面修改类型
- **AND** 单个、非法或重复 IMEI 仍保持待确认；明确 `tablet` 或同时命中手机/平板的冲突类型不被覆盖

#### Scenario: Apple 手机品牌优先于设备类型手机

- **WHEN** 报告“手机品牌”规范化后精确为 `iPhone` 或 `iPad`，而“设备类型”写为“手机”
- **THEN** 系统分别将检材确认为 `phone` 或 `tablet`，并记录 Apple 品牌分类诊断
- **AND** Apple 品牌分类优先于双 IMEI 与设备类型手机；非精确 Apple 品牌值不得触发该规则
- **AND** 在没有 Apple 品牌、明确平板或类型冲突时，双 IMEI 先于设备类型手机生效；设备类型手机仍可作为最后的报告自动候选
...
正常情况下主取证软件由报告适配器自动识别；已确认的版本化内置适配器 MAY 使用用户明确批准的厂商/格式专属默认名称，并将其标记为 `confirmed_by_user`，但不得扩展到未命中该适配器的报告。无法可靠识别且没有已确认适配规则时，报告 MUST 进入审核页面并将软件名称和版本标记为待确认，用户可以人工填写或修正。主软件确认前允许审核和编辑，但 MUST 阻止最终正式导出；不得使用无格式边界的历史固定软件、不得从普通组件猜测，也不得仅生成 WinRAR 和 Python hashlib 后将文书视为完整。

阶段一系统 MUST 从报告来源或已确认的版本化适配规则得到主取证软件名称，版本仍 MUST 来自报告；不得用无格式边界的硬编码名称或环境检测值覆盖它。`softwareTools` 只允许包含主取证软件、WinRAR 和 Python hashlib 三类工具；WinRAR/Python 的版本可以来自执行环境。适配规则提供名称时 MUST 保留用户确认来源，不能伪装成报告原始字段。
...
#### Scenario: 已确认适配规则补充报告缺失的软件名称

- **WHEN** 报告未提供主取证软件名称，但唯一命中的版本化内置适配器包含用户明确批准的格式专属默认名称，且报告提供有效版本
- **THEN** 规范化模型、正文和工具列表使用该默认名称与报告版本，并把名称确认状态标记为 `confirmed_by_user`
- **AND** 未命中该适配器的报告、其他厂商报告和未知变体不复用此名称
...
系统 MUST 先生成 `Attachment1Plan` 再渲染附件一。数据行数 MUST 等于最终 `ArchiveManifest` 的卷数；每个数据页最多三项，并从前向后优先填满三项。最后一个数据页 MUST 只补足到三个分卷槽位：末页少于三卷时，紧接最后一卷的第一条补位行在“电子数据”列显示“以下空白”，其余补位行保持空白，所有补位行均不得使用旧斜线；末页恰好三卷时第三卷后直接进入固定手写签名区，不得生成第四个“以下空白”行。固定手写签名区位于数据区或补位区之后并贴近页面底部；表头只在第一页出现且不得由 Word 自动重复；“附件1”只在第一页；来源框和提取方法框每个数据页分别生成合并框。最后一页 MUST 同时包含至少一条分卷数据和固定手写行，不得生成只含固定手写行的独立页面。规划器 MUST 以每页最多三项、优先填满前页的确定性分页和末页补位规则为固定手写行预留实际版式容量，不得为均衡末页而减少前页条数，也不得依赖 Word 自动分页后留下孤立签名页。固定手写行和其页面 MUST 设置不可拆页/保持完整，不写入动态检查人员。

#### Scenario: 页面规划跨页边界

- **WHEN** 页面规划器接收当前系统正式上限的、已验证的五条 part 记录
- **THEN** 页面计划有两页，行数为 5，第一页 3 行并显示表头和“附件1”，第二页 2 行且不重复表头；第二页在第五卷后生成一条“以下空白”补位行，再在页底生成检查人员框；两页均有各自来源合并框和提取方法合并框

#### Scenario: 四卷附件一按三加一分页且不生成独立签名页

- **WHEN** 已验证的 `ArchiveManifest` 包含四条实际分卷
- **THEN** 页面计划生成两页，第一页包含前三条分卷，第二页包含第四条分卷并在其后附带固定手写行
- **AND** 不得生成不含分卷数据、只含固定手写行的页面
...
- **WHEN** 最终 `ArchiveManifest` 包含至少三条分卷，且最后一页少于三条数据
- **THEN** 最后一个数据页在最后一卷后生成“以下空白”行和所需普通空白补位行，所有补位行均不添加斜线

#### Scenario: 四卷附件一避免签名行被挤成孤页

- **WHEN** 已验证的 `ArchiveManifest` 包含四条实际分卷
- **THEN** 页面计划生成两页并按 `[3,1]` 排列，第二页包含第四条分卷并在其后附带固定手写行
- **AND** 附件摘要页数必须与最终物理页数一致，不得把固定手写行单独留在下一页

#### Scenario: 末页签名区固定在补位区之后的页面底部

- **WHEN** 已验证的 `ArchiveManifest` 包含一至五条实际分卷
- **THEN** 末页少于三卷时，最后一卷的下一行在“电子数据”列显示“以下空白”，并以普通空白行补足末页三个分卷槽位；末页恰好三卷时不生成该补位行
- **AND** 固定手写签名区位于所有补位行之后并贴近末页底部，不得紧跟最后一卷向上浮动、越出页面或成为独立页

#### Scenario: SHA-256 保持原字号和相同分页

- **WHEN** 案件选择 SHA-256 且 `ArchiveManifest` 包含一至五条64位摘要
- **THEN** 附件一表头显示“文件SHA-256哈希值”，每个数据单元格完整显示对应64位摘要并移除模板遗留空段落
- **AND** 哈希文字保持与 MD5 相同的16磅字号，只允许通过无重叠的字符宽度调整适配列宽；3/4/5卷仍分别按 `[3]`、`[3,1]`、`[3,2]` 排列，签名区不得成为独立页
...
### Requirement: 平航手机多路取证报告 v1 受控适配

系统 MUST 将当前已确认结构的平航手机多路取证离线网页报告作为版本化内置适配器 `pinghang-mobile-multipath-v1` 接入报告目录选择、来源登记和解析链路。适配器 MUST 通过入口 HTML、`报告/data/navigation_data.js`、`报告/data/ViewData/*.json`、ViewData 固定赋值前缀、`Info.type` 和字段标签集合联合识别，不得只按目录名、案件名、文件编号或厂商文字匹配。该内置适配只承诺当前结构版本；未知变体不得静默套用。

ViewData 与导航内容 MUST 作为不可信数据字面量读取。系统 MUST 验证允许的赋值/节点语法，并对导航和被选核心元数据施加文件大小、选中数量、内存、耗时和嵌套限制；不得用报告总容量或非核心 ViewData 总页数阻止案件初始化。系统 MUST NOT 通过 `eval`、`exec`、Node VM、浏览器或其他脚本宿主执行报告内容。探测和案件事实解析 MUST NOT 枚举、打开或递归读取未被导航/版本化锚点选中的 ViewData、附件或其他大量业务数据；报告的 `附件/图片文件` 和默认设备图片 MUST NOT 自动成为文书附件二检材照片。

平航 v1 MUST 使用快速核心路径：报告信息页由固定锚点 `0_1.json` 加赋值变量、`Info.type` 和标签集合共同确认；案件页由导航中 Base64 显示名为“案件信息”的唯一 Table 节点定位；材料页由全部 `DeviceInfo` 节点按导航顺序定位；同一顶层设备作用域内唯一显示名为“机主信息”的 Table MAY 作为该材料的持有人页。持有人页只能读取明确“用户姓名”并映射为 `holder_name`，不得使用电话号码、证件信息或其他人员字段猜测；多个材料或多个机主信息页落入同一作用域而无法唯一配对时必须安全失败。不得只凭固定编号接受页面；被选核心页结构不符时必须失败，未选中的聊天、动态或媒体页不得被解码，也不得因其体量、编码或数据字面量差异阻塞案件初始化。

适配结果 MUST 将案件信息（含“送检人员”“送检单位”）、报告完成时间、设备材料、持有人、标识符、主软件和字段来源映射到 `CanonicalInspectionCase`/`FieldProvenance`，再投影现有 `InspectionReport` 供当前审核和导出链路使用。“送检人员”“送检单位” MUST 分别进入现有委托人员和委托单位。平航 v1 的主取证软件名称 MUST 按用户确认的内置适配规则默认为“平航手机多路分析取证软件”并标记为 `confirmed_by_user`，版本 MUST 取报告“数据取证软件版本”；该默认值不得用于未命中 v1 的报告。平航检查起止时间 MUST 在每个材料起止时间均有效且不倒置时，取全部材料中最早“取证开始时间”和最晚“取证结束时间”；否则留空待审核，不得用只有日期的报告完成日期补造分钟。案件创建时间与报告完成时间仍作为独立来源事实保留。报告未明确绑定取证硬件时，平航解析初值 MUST 为空或使用后续明确的用户设置，不得套用美亚硬件默认值。平航 v1 `DeviceInfo` 必须优先使用明确“设备类型”或“检材类型”，仅有“数据类型=Android设备”时不得归类为手机；缺少可靠类型时统一适用受控双 IMEI 兜底，其余类型允许审核修正并在确认前阻止正式导出。
...
#### Scenario: 当前结构的单检材平航报告正常解析

- **WHEN** 用户选择一份命中 `pinghang-mobile-multipath-v1` 全部结构签名的单检材报告目录
- **THEN** 系统登记来源并生成包含案件、送检人员/单位、一个材料、可唯一绑定的持有人、设备标识符、软件版本候选、基于设备取证起止时间的检查时间和 provenance 的审核草稿
- **AND** 当前报告格式和现有 Word/归档输出合同保持不变
...
#### Scenario: 平航 Android 数据类型不冒充手机类型

- **WHEN** 命中 v1 的平航材料页仅有“数据类型”为 `Android设备`，没有明确“设备类型”或“检材类型”
- **THEN** 系统保留原始值但不据此归类为 `phone`
- **AND** 仅当 IMEI1 与 IMEI2 均为有效、不同的 15 位数字时才按统一兜底规则推断 `phone`，否则保持待确认
- **AND** 同一材料存在不同的明确类型值时保留冲突并要求人工确认，不按字段顺序取首值

#### Scenario: 多个设备页面按导航顺序形成材料

- **WHEN** 报告包含多个有效且检材编号唯一的 `DeviceInfo` 页面，并且导航为这些页面提供唯一稳定顺序
- **THEN** 系统按导航顺序生成 `Material[]`，每个材料只绑定自身字段和 provenance
- **AND** 不以 ViewData 文件枚举顺序或设备型号猜测材料顺序

#### Scenario: 多检材取证时间形成检查时间

- **WHEN** 平航报告包含多个材料，且每个 `DeviceInfo` 页面均提供有效且不倒置的“取证开始时间”和“取证结束时间”
- **THEN** 检查起止时间取全部材料中最早的取证开始时间和最晚的取证结束时间
- **AND** 任一材料缺失、无法解析或起止倒置时不自动生成部分时间范围，审核页面保持待补充状态

#### Scenario: 平航报告结构或字段存在歧义

- **WHEN** 内置适配器未命中、多个适配器并列命中、导航无法确定材料顺序、语义页面重复冲突或检材编号重复
- **THEN** 系统返回稳定安全的“不支持或结构歧义”诊断，允许用户重新选择或保留可审核候选时继续编辑
- **AND** 在歧义消除前不得正式导出，不得回退到第一个成功解析器

#### Scenario: 报告脚本和提取图片不越过输入边界

- **WHEN** 平航报告包含网页脚本、工具程序、提取图片、未知附件或畸形 JSONP 数据
- **THEN** 系统不执行其中任何脚本或程序、不把提取内容自动绑定为检材照片，并只读取 profile 允许的有界元数据
- **AND** 非法赋值、额外语句、超限内容或路径越界使解析安全失败且不泄漏绝对路径和原始敏感值
