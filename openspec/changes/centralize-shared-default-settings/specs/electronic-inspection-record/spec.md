...
系统 MUST 通过部署实例作用域的后端事实源维护笔录共享默认值，并在统一平台外壳中提供独立管理入口。案件审核编辑只修改当前案件，不得隐式更新笔录默认设置。
...
- **AND** 页面不展示或提交委托单位前缀、光盘编号前缀及附件1提取方式，不展示当前版本及案件基础信息、文件哈希算法、检查人员顺序下方的说明提示
...
- **WHEN** 用户在案件审核编辑页修改文号、检查地点、检查方法、检查硬件设备、检查要求、检查人员、附件1提取方式或光盘编号并保存草稿
- **THEN** 修改只进入当前案件草稿
