...
#### Scenario: Word 内容预览与当前对话使用可交换分栏
...
- **AND** 页面提供明确的左右位置交换操作；交换后 Word 内容预览与当前对话同步改变视觉顺序、DOM 顺序和键盘顺序，但不得卸载当前回复、清空本次会话展示状态或修改任何业务数据
- **AND** 用户交换后的分栏顺序作为浏览器本地界面偏好保存，刷新页面或打开其他案件时继续沿用；存储不可用或保存值无效时回退为 Word 内容预览在左，且该偏好不得写入案件草稿、报告或服务端
...
- **AND** 窄屏和高倍浏览器缩放下分栏改为单列并取消不必要的嵌套滚动；同一交换操作改为切换上下顺序，文字、结构化控件、键盘焦点与主要操作不得被裁切或遮挡
...
- **THEN** 页面在下方当前对话区推荐一个当前事项，同时以带 Tooltip 和明确可访问名称的圆形图标工具栏提供“已填内容与待办”和“返回案件列表”；不提供独立“查看已整理信息”入口，完整审核编辑作为“已填内容与待办”面板内的兜底修改入口，当前回复或确认始终是唯一主操作
...
- **AND** 除“选择压缩时机→填写介质编号”作为正常办理的明确前置顺序，以及“委托时间→案件简要情况”作为绪论字段的相邻顺序外，助手不得为文号、检材、图片等其余字段采用固定题序；已由报告、默认设置或用户操作满足的事项直接从操作集中消失，但报告解析出的案件简要情况仍须在本次页面会话中以预填多行输入框提供一次人工核对和修改，确认后不重复询问
