...
阶段一的最终检材类型只允许 `phone` 或 `tablet`。`ReportAdapter` 可以返回候选类型；报告明确且可靠提供类型时可以自动选择。缺少可靠类型时，只有 IMEI1 与 IMEI2 均为有效、不同的 15 位数字，系统才可兜底推断 `phone` 并记录推断诊断；单个、非法或重复 IMEI 不得推断，明确 `tablet` 或类型冲突不得被该兜底覆盖。其余无法可靠判断的材料 MUST 保持待确认并由审核页面确认；每个检材在最终导出前 MUST 完成类型确认。原始 identifiers MUST 保留在 `CanonicalInspectionCase` 中，显示规则只能控制模板可见性。
