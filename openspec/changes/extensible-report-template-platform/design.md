...
| 检材名称、编号、型号、标识符、类型、取证时间 | `Info.type = DeviceInfo` | 每个页面形成一个材料候选，保留 IMEI/IMEI2/序列码来源；用户确认将明确“数据类型”值 `Android设备`（兼容大小写、全半角和空白差异）归类为手机；全部材料时间有效时，以最早“取证开始时间”和最晚“取证结束时间”形成检查起止时间 |
...
多个 `DeviceInfo` 页面按已验证的导航顺序生成 `Material[]`；导航缺失、页面重复、检材编号重复或顺序无法确定时保留候选并阻止正式导出，不能按文件枚举偶然顺序猜测。平航 v1 将报告明确“数据类型”规范化后恰为 `Android设备` 的材料归类为手机并产生 `confirmed_by_report`；该映射来自用户确认的内置适配规则，仅吸收大小写、全半角和空白差异，不根据 IMEI、目录名或设备型号推断，也不扩展到其他报告格式或未知类型值。平航检查起止时间按用户确认的报告语义取全部材料中最早的“取证开始时间”和最晚的“取证结束时间”；只有每个材料的起止时间均可解析且不倒置时才自动投影，否则留空待审核，不使用只有日期的报告完成日期补造分钟。案件创建时间和报告完成时间仍作为独立来源事实保留。报告未明确绑定取证硬件时，平航解析初值保持为空并由现有审核/共享默认设置补充，不得套用美亚硬件默认值。报告本身未提供主软件名称时，已确认的 v1 适配规则使用“平航手机多路分析取证软件”作为用户确认默认值，并继续把报告中的“数据取证软件版本”作为版本事实；该规则不得扩展到未命中 v1 的平航变体或其他厂商报告。
