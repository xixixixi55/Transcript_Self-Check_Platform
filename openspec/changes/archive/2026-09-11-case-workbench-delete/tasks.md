...
spec_sync_evidence: 删除/归档入口的最终阶段语义、DELETE 清理合同与 data-model 已按 T010、T017、T018 同步到 living specs。
...
- [x] T017 将案件工作台原“删除案件”入口及确认操作的显示名称统一改为“归档案件”，保持原删除回调、DELETE API、清理范围和不可恢复提示不变；更新前端回归，同步相关 delta 与 living spec，并运行 Level 2 风险相称门控。
...
- [x] T019 按用户反馈将已导出案件通过“归档案件”入口操作成功后的提示改为“案件已归档。”；普通“删除案件”入口继续提示“案件已删除。”，保持原 DELETE API、清理范围和刷新行为不变。
  - 本次增量为 Level 1 文案修正，关联 T017/T018，不新增或修改正式接口、持久化格式及删除行为。
  - 验证：案件工作台页面测试 16/16 通过，分别断言普通删除成功提示“案件已删除。”与已导出案件归档成功提示“案件已归档。”；前端及共享类型检查通过，`git diff --check` 通过。人工验收 N/A（提示文案由 SYNTHETIC 页面回归直接覆盖）。
