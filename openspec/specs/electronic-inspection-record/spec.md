...
- **AND** 用户可以从“已填内容与待办”进入任一当前待处理事项、返回本次会话已完成的既有结构化控件，并查看由当前草稿事实恢复、且确由用户通过獬豸助手直接填写或确认的此前处理字段及现值；文号、委托时间、检材信息、介质编号、刻录时间等此前处理内容必须可点击，点击后仍停留在獬豸助手并重新打开对应的原结构化回复控件，同时纳入本次会话的前进/后退轨迹，不得跳转到完整审核编辑。检查步骤、系统产物及其他由识别、模板或规则生成而非用户直接办理的内容不得进入“此前已处理”。正在处理或本次刚完成的同一字段不得重复展示。用户仍可通过“修改其他已填内容”进入完整审核编辑，不被强制锁定在固定的一题一页顺序中
