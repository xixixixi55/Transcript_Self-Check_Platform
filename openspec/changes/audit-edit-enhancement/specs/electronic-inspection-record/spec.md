# 规格增量：电子数据检查笔录自动生成——审核编辑界面增强

> 基准 Spec: `openspec/specs/electronic-inspection-record/spec.md`
> 变更类型：MODIFIED (CAP-003 REQ-007) + ADDED (CAP-010, CAP-011)

## ADDED: REQ-034 — 软件名称净化与检材可提取状态

系统 MUST 从可靠的软件名称/版本候选中移除表示取证塔、取证设备、取证工作站、采集设备或硬件设备的括号描述，保留报告识别到的软件名称和版本，不得擅自映射为其他产品名称或将硬件名称写入软件工具名称。

系统 MUST 在报告解析时根据每项检材的 IMEI1、IMEI2、序列号是否至少存在一个非空值生成“是否可提取”状态，并在审核编辑界面展示且允许人工修正。用户将检材设为无法提取时，界面 MUST 显示无法提取原因输入框；原因随案件草稿保存，留空时 MUST 作为待核对项提示。无法提取的检材 MUST 隐藏 IMEI/序列号，Word 检材情况和检查过程步骤 1 MUST 使用用户填写的原因替代设备标识。缺少新字段的存量数据 MUST 使用同一标识规则兼容推导，并在没有原因时保留既有“无法提取”兜底文案。

**Scenario: 软件名含硬件括号描述**
- WHEN 报告可靠候选为“手机大师-并行版V5（FL-901 手机取证塔V5） V3.2.12922”
- THEN 软件工具显示“手机大师-并行版V5（版本号为V3.2.12922）”
- AND 不显示硬件括号描述

**Scenario: 其他软件名含硬件括号描述**
- WHEN 任意可靠软件候选的括号内容包含受控硬件语义
- THEN 仅移除该硬件括号内容
- AND 保留软件名称中其他非硬件括号内容及独立版本号

**Scenario: 报告识别到手机大师NEXT**
- WHEN 报告可靠候选为“手机大师NEXT（FL-901 手机取证塔V5） V1.1.22111”
- THEN 软件工具显示“手机大师NEXT（版本号为V1.1.22111）”
- AND 不映射为“手机大师-并行版V5”

**Scenario: 标识存在时自动判定可提取**
- WHEN 检材的 IMEI1、IMEI2 或序列号至少一项非空
- THEN 解析结果标记该检材可以提取
- AND 审核页显示“可以提取”及其类型允许展示的标识

**Scenario: 标识全部缺失时自动判定无法提取**
- WHEN 检材的 IMEI1、IMEI2 和序列号全部为空
- THEN 解析结果标记该检材无法提取
- AND 审核页不显示 IMEI 或序列号编辑项
- AND Word 检材情况显示“设备名一部（无法提取）”
...
**Scenario: 失焦保存**
- WHEN 工作人员在编辑状态下点击字段外部区域或按下 Enter
...
**Scenario: 编辑结构化条目文本**
- WHEN 工作人员编辑检材、检查人员、软件工具或提取清单中的文本
- THEN 该文本先以展示态呈现，点击后进入编辑态
- AND 保存、取消、添加和删除操作保持对应编辑器的既有数据回调行为

**Scenario: 人工检材同步到检查过程和检查结果**
- WHEN 工作人员人工新增、删除、改号或调整检材顺序
- THEN （三）检查过程中的检材描述、拍照步骤和检查步骤立即按当前检材列表重新生成
- AND （四）检查结果的检材编号按当前有效检材编号依序使用“、”拼接
- AND 环境检查等不依赖检材的步骤保持原内容

**Scenario: 光盘编号使用统一入口**
- WHEN 工作人员进入审核编辑页面，且案件尚未开始压缩或正在压缩
...
- AND 区域前缀从委托单位自动推断（如测试单位名称→"测试公"，否则"xx"）
...
## ADDED: CAP-014 — 审核提示与正式文书规范化

### REQ-032: 案件简要、校验值、来源、版本和 Word 标题必须规范化

**Scenario: 案件简要提示人工核对**
- WHEN 用户审核案件简要情况
- THEN 页面提示该内容由报告解析生成、可能不准确并需要人工核对
- AND 当内容末尾存在空格、制表符或换行时额外提示删除多余空白

**Scenario: MD5 使用大写形式**
- WHEN 页面展示、审核编辑或 Word 导出 MD5 值
- THEN 十六进制字母统一使用大写形式
- AND 内部校验仍按不区分大小写的同一摘要处理

**Scenario: 固定清单来源包含检材语义**
- WHEN 系统根据检材编号生成电子数据提取固定清单来源
- THEN 来源格式为“<检材编号>检材内提取”
- AND 多个检材编号保持既有顿号顺序后再追加“检材内提取”

**Scenario: 检查步骤不重复具体版本号**
- WHEN 系统根据主取证软件名称和版本生成检查过程步骤 4
- THEN 具体版本只出现在“（版本号为<版本>）”中一次
- AND 软件名称自身包含产品代际标识时保持原名称不变

**Scenario: Word 标题使用规定格式**
- WHEN 系统生成电子数据检查笔录 DOCX
- THEN 主标题“电子数据检查笔录”居中且不加粗
- AND “电子数据提取固定清单”标题加粗

**Scenario: Word 附件摘要保留附件标识**
- WHEN 系统根据当前附件计划更新 Word 附件摘要
- THEN 第一项必须以“附件：1、电子数据提取固定清单”开头
- AND 后续附件项保持原有编号和内容

**Scenario: Word 附件摘要按剩余空间条件分页**
- WHEN 检查结果结束后留出三个空白行，当前页仍能完整容纳三条摘要、检查人签名区、双横线和日期
- THEN 完整附件摘要区域必须在三个空白行后继续排在当前页
- WHEN 当前页在三个空白行后不能完整容纳上述区域
- THEN 完整附件摘要区域必须整体移动到下一页并从该页开始排版
- AND 三个空白行只作为摘要与检查结果同页时的间隔，摘要独立成页时不得在该页顶部保留这三个空白行或等效留白
- AND 三条摘要、检查人签名区、双横线和日期不得跨页拆分
- AND 独立页中的三条摘要、检查人签名文字、双横线和日期必须保留变更前模板的缩进、行距、对齐及纵向位置
- AND 附件一、附件二和附件三正文继续使用既有分页规则

**Scenario: 附件一短表头使用紧凑格式**
- WHEN 系统生成附件一“电子数据提取固定清单”表格
- THEN “电子数据”和“来源”表头文字居中且字符连续排列
- AND 不继承导致字符分散的缩进、字符间距或文字缩放属性

**Scenario: 附件一来源正文使用两端对齐**
- WHEN 系统生成附件一“电子数据提取固定清单”数据行
- THEN “来源”列正文段落使用两端对齐
- AND 来源表头继续居中，逐检材编号换行、末尾“检材内提取”和垂直合并规则保持不变

---

## ADDED: CAP-015 — 审核检材类型投影到 Word

### REQ-035: Word 检材情况包含已确认检材类型

审核编辑界面保存的检材类型 MUST 参与正式 Word 的检材情况渲染。设备品牌型号与类型使用连续文本组合；已经包含同一中文类型名称时不得重复追加。

**Scenario: 手机类型追加到品牌型号**
- GIVEN 审核编辑界面的设备名称为“HUAWEI SGU-AL10”
- AND 检材类型为手机
- WHEN 系统生成正式 Word
...
- AND 设备名称已经以“平板”结尾时不得生成重复的“平板平板”
