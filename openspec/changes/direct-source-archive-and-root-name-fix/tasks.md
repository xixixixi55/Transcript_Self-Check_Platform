...
- [x] T004 将归档编排改为直接源 inventory 与 WinRAR 前后变化门控。
  - 文件：`packages/backend/app/services/archive_execution_service.py`、`packages/backend/app/repository/archive_input_repository.py`、`packages/backend/app/repository/archive_attempt_recovery_repository.py`、`packages/backend/app/repository/winrar_execution_models_repository.py`
...
- [x] T005 保留历史 snapshot 恢复与清理兼容。
  - 文件：`packages/backend/app/services/archive_attempt_recovery_reconciliation_service.py`、`packages/backend/app/services/archive_input_snapshot_recovery_service.py`、案件删除相关测试
