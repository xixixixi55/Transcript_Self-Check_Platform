...
| 字段 | 数据来源 | 映射到笔录 |
|------|---------|-----------|
| 案件名称 | contents[tp=案件名称] | CaseDraft 案件名称；不填充一(四)案件简要情况 |
...
- **THEN** 系统 MUST 删除末尾括号及括号内内容，并将清理后的 `xx案` 仅用于 CaseDraft 案件名称；案件简要情况保持为空
- **AND** 清理仅针对案件名称末尾的括号标记，不删除名称中部内容

#### Scenario: 案件简要情况不复用案件名称
- **WHEN** 报告识别到任意案件名称
- **THEN** 系统 MUST 保留规范化后的案件名称作为 CaseDraft 案件名称，并将案件简要情况初始化为空
- **AND** 系统 MUST NOT 将案件名称复制到案件简要情况，也不得自动补充“案”字
...
系统 MUST 为每项检材保留独立的可选持有人姓名。new 报告仅从该检材设备行内结构化的“检材持有人姓名”字段读取；legacy 报告仅从对应检材 `Base`/`Phone` 设备元数据中结构化的 `c1="检材持有人"`、`c2=<姓名>` 键值行读取。缺少对应明确字段时保持为空，不得从通讯记录、联系人、案件名称或其他非权威内容推断。审核界面 MUST 以“持有人”展示并允许原位修改，修改随现有案件草稿和字段来源链路保存。
...
#### Scenario: legacy 报告按检材解析持有人

- **WHEN** legacy 报告对应检材的 `Base`/`Phone` 设备元数据包含结构化 `c1="检材持有人"`、`c2=<姓名>` 键值行
- **THEN** 系统把该姓名写入同一检材的 `holder_name`
- **AND** “持有人编号”“持有人民族”“持有人性别”等相邻字段不得被误识别为姓名

#### Scenario: legacy 或 new 报告缺失明确持有人字段

- **WHEN** 对应检材没有该格式所规定的结构化持有人字段
- **THEN** 对应检材的持有人保持为空并可由用户手动补充
- **AND** 系统不得从通讯记录中的“机主”等文本或其他模糊来源推断持有人
...
系统 MUST 满足以下现有合同。工作台编辑通过后端自动保存并携带草稿 revision；编辑会话使用心跳租约，连续无心跳达到既定超时后才允许用户确认接管。版本冲突、租约冲突和保存失败不得静默覆盖后端草稿。系统通过部署实例作用域的后端事实源维护笔录共享默认值，并在统一平台外壳中提供独立管理入口；案件审核编辑只修改当前案件，不得隐式更新笔录默认设置。
...
- WHEN 用户进入“笔录默认设置”页面
- THEN 系统展示当前文号格式、检查地点、检查方法、检查硬件设备、数据摘要、检查要求、有序检查人员和文件哈希算法
- AND 文号格式由编号前内容和编号后内容组成，并展示使用示例；设置页不要求用户输入技术占位符
- AND 页面不展示或提交委托单位前缀、光盘编号前缀及附件1提取方式，不展示当前版本及案件基础信息、检查人员顺序下方的说明提示
...
#### Scenario: 委托单位前缀能力彻底废除
- GIVEN 旧案件、旧默认值或旧版请求中仍携带 `entrust_unit_prefix`
- WHEN 系统读取、编辑、预览或生成 Word
- THEN 系统忽略并在规范投影中丢弃该旧字段，只使用 `entrust_unit`
- AND 新案件不从共享默认值初始化该字段，当前 API 和共享类型不再提供该字段
- AND 共享默认值写入请求携带该字段时按未知字段整体拒绝
- AND 模板正式导出与 batch 兼容导出均不得把旧前缀写入 Word
...
- WHEN 用户在案件审核编辑页修改文号、检查地点、检查方法、检查硬件设备、检查人员或光盘编号并保存草稿
- THEN 修改只进入当前案件草稿
- AND 草稿请求不生成或提交 `shared_defaults_patch`
- AND 当前笔录默认设置保持不变

#### Scenario: 点击字段进入编辑
- WHEN 工作人员在预览页面上点击任意文本字段
...
#### Scenario: 修改案件简要情况
- WHEN 工作人员编辑案件简要情况（自由文本）
- THEN 预览实时更新

#### Scenario: 修改检查设备硬件
- WHEN 工作人员从硬件下拉框选择不同设备
- THEN 检查设备章节自动更新
...
- AND 未提交导出目录的一般 Legacy 兼容请求继续触发浏览器下载，但审核页不暴露该兼容入口

#### Scenario: 导出位置随案件报告确定
- WHEN 工作人员从工作台执行完成导出并确认文件名
...
#### Scenario: 归档完成后完成导出到用户路径
- WHEN 案件进入归档完成态且工作人员在工作台点击“完成导出”
- THEN 系统把最新编辑数据生成的 Word 与全部 RAR 文件写入案件报告文件夹的上一级目录
- AND 本次完成导出的位置仅由该案件报告来源决定
...
- AND 检查笔录完成导出不得启动 HashMyFiles 或生成校验截图；现有三算法截图能力保留，等待鉴定文书模块接入
- AND 立即压缩生成的 RAR 已位于报告上级目录，完成导出只生成 Word 并复用 RAR；仅旧版工作区产物仍走兼容迁移，登记成功后清理旧原件
...
- AND 导出成功后工作台保持可用，工作人员可再次打开案件修改并重新完成导出

#### Scenario: 导出后仍可修改
- WHEN 导出完成后
- THEN 工作台保持可用，工作人员可再次打开案件修改并重新完成导出
...
#### Scenario: 机械盘大体积完成导出不被固定请求上限提前截断
...
- AND 普通工作台请求仍使用其独立短超时，不得随完成导出一起放宽
...
- AND “检查人员：”从签名行左侧内容边界开始，落款名称的末字与右侧内容边界对齐
- AND 落款名称超出单行可用宽度时由 Word 自动换行，不得依赖固定空格定位
...
#### Scenario: 新增带所属公司的设备
- WHEN 工作人员填写设备名称和非空所属公司并保存
- THEN 系统持久化这两个字段并在刷新后的设备列表中显示
- AND 该设备继续出现在审核编辑界面的硬件设备下拉框中

#### Scenario: 所属公司为空
- WHEN 工作人员新增设备或编辑旧设备但提交的所属公司为空或仅包含空白
...
检查步骤 4 MUST 同时记录启动主取证软件和使用该软件处理全部检材两个动作，格式为“启动{软件名称}软件，使用该软件对检材{全部检材编号}进行检查。”；软件名称已以“软件”结尾时不得重复追加。检查过程和检查结果叙述 MUST 不显示主软件版本号，软件工具清单 MUST 继续显示主软件及其他工具的实际版本。软件名称和有序检材编号 MUST 来自当前案件结构化数据，不得写死产品、版本或检材编号。

最终 Word 的“（四）检查结果”正文 MUST 使用两个汉字宽度的首行缩进；不得以一个汉字宽度缩进或依赖正文开头的字面空格模拟段落格式。
...
- AND 使用动作以“该软件”指代已启动的软件
- AND 检查步骤 4 和检查结果不显示版本号
- AND 软件工具清单仍显示“美亚手机大师-并行版V3（版本号为V3.2.08602）”
- AND 最终 Word 使用当前案件保存的同一步骤内容，不退回只描述启动软件的旧句式

#### Scenario: 检查结果使用双字符首行缩进
- WHEN 系统生成最终 Word 的“（四）检查结果”正文
- THEN 该段首行缩进为两个汉字宽度
- AND 正文保留主软件名称但不显示主软件版本号
...
系统 MUST 在报告解析时仅按每项检材 IMEI1、IMEI2 是否至少存在一个非空值自动生成“是否可提取”状态，并在审核编辑界面展示且允许人工修正；序列号继续解析、保存、展示和允许修改，但不参与自动判定。用户将检材设为无法提取时，界面 MUST 显示无法提取原因输入框，并继续提供 IMEI1、IMEI2 和序列号的修改入口；原因随案件草稿保存，留空时 MUST 作为待核对项提示并阻止审核编辑界面的 Word 导出。无法提取时，正式 Word 检材情况和检查过程步骤 1 使用用户填写的原因替代设备标识。缺少新字段的存量数据按同一规则兼容推导，显式保存的人工布尔值优先，并在没有原因时保留既有“无法提取”兜底文案。

#### Scenario: 无 IMEI 的检材
- WHEN IMEI1 和 IMEI2 全部为空，无论序列号是否存在
- THEN 检材自动标记为无法提取
...
- THEN 原因随案件草稿保存
- AND 审核界面继续显示并允许修改 IMEI1、IMEI2 和序列号
...
獬豸助手 MUST 在“软件工具”结构化控件中只展示一次主取证软件名称和版本，并将主软件确认状态、名称和版本编辑入口合并到同一主软件行；不得重复展示独立的主取证软件区块。该展示合并 MUST 继续使用 `inspection.primary_software` 作为唯一权威编辑结构，不改变现有派生字段、检查步骤或正式导出门控。

#### Scenario: 已识别主软件只展示一次

- **WHEN** 报告已识别主取证软件名称和版本，且 `software_tools` 包含对应主软件条目
- **THEN** 獬豸助手只在“软件工具”的主软件行显示一次该名称和版本
...
每个可编辑叶子字段、检材字段、人员项、附件图片组和需要跨页面会话保留的人工确认 MUST 有 `FieldState`，包含稳定字段路径、来源 `report | user | system_default`、确认状态 `confirmed | pending` 和 revision。纯派生不可编辑字段继承来源，不单独维护状态；来源颜色不得进入 Word，pending 必须有文字提示。审核编辑页 MUST 不显示单独的“文号来源”行或页面级“字段来源”说明块。
...
#### Scenario: 案件简要情况确认跨页面会话保留
- WHEN 用户在獬豸助手中确认报告已整理的案件简要情况并保存案件，随后重新打开该案件
- THEN 草稿通过既有 `FieldState` 保留该人工确认，不再重复生成案件简要情况待确认事项
- AND “已填内容”显示案件简要情况及当前值，点击后仍在獬豸助手中打开原文本核对控件
- AND 该确认状态不进入报告正文或 Word 导出内容
...
- AND `archive_complete` 在工作台展示为「待导出」并推荐统一导出，`exported` 只展示已导出最终结果并推荐归档案件
...
案件工作台 MUST 是电子检查笔录的主生产入口，使用既有 Legacy `InspectionReport` 字段合同、校验规则、日期时间处理、附件模型、预览投影和 Word 导出映射。案件内审核只使用獬豸助手，不得维护完整审核编辑器或简化的第二编辑器；工作台负责案件状态、来源、多案件控制与完成导出。后端 `/records/*` 保留为 Legacy 兼容入口和唯一正式 Legacy 输出管线，不构成第二个持久化工作台流程。

#### Scenario: 獬豸助手完整承接审核编辑
- WHEN 案件达到 `review_ready`
- THEN 案件审核页通过獬豸助手及 Word 内容预览暴露全部适用 Legacy 审核字段、数据摘要、附件信息、图片编辑、必填/格式校验和预览
- AND 案件审核页不提供单独 Word 或完成导出入口；完成导出与自定义文件名只在案件工作台提供
...
#### Scenario: 案件卡片展示案件和委托信息
- WHEN 案件已解析出案件名称、委托人和委托单位
- THEN 案件卡片标题显示案件名称
- AND 卡片辅助信息显示带标签的委托人和委托单位
- AND 卡片不显示案件编号
- WHEN 案件仍在解析或报告未提供委托人或委托单位
- THEN 案件名称标题继续使用既有待解析兜底，缺失的委托信息分别显示待解析兜底

#### Scenario: 案件卡片各阶段不重复解释既有状态和操作
...
#### Scenario: 案件删除或归档先确认后执行既有删除能力
- **WHEN** 用户点击案件卡片的“删除案件”操作（统一导出成功后主按钮显示为“归档案件”）
- **THEN** 未导出案件弹出“确认删除该案件？”并使用“确认删除”按钮，正文以“删除后”说明不可恢复范围；统一导出成功后的归档入口保留“确认归档该案件？”与“确认归档”
- **AND** 仅统一导出成功后直接显示“归档案件”推荐操作；其他状态的更多菜单显示“删除案件”
- **AND** 已导出案件的确认提示明确说明已导出到目标目录的文件不会被删除
- **WHEN** 用户点击“确认删除”（已导出案件为“确认归档”）
...
#### Scenario: 案件工作台移除来源目录校验和手动刷新
- **WHEN** 用户进入案件工作台
- **THEN** 页面不显示来源目录校验开关和手动刷新按钮
- **AND** 上传目录、任务自动更新、错误重试和案件卡片操作继续可用
...
#### Scenario: 审核编辑不展示委托单位前缀
- **WHEN** 用户打开案件的 Word 内容预览并审核委托单位信息
- **THEN** 页面只展示可编辑的“（一）委托单位”，不展示或提供“委托单位前缀”编辑项
- **AND** 委托单位继续独立编辑和保存；旧前缀数据被忽略，正式 Word 只输出委托单位
...
- AND 卡片以“完成导出”为唯一推荐操作；提示只在盘号补齐后出现，未补齐时保持“待补盘号”并推荐打开案件
- AND “待导出”阶段的更多菜单同时提供“打开案件”与“删除案件”；打开案件进入獬豸助手审核路由，不触发完成导出，也不改变案件状态

#### Scenario: 导出成功后标记已导出
- WHEN 完成导出写入用户路径成功
- THEN 案件卡片标记为「已导出」
- AND 「已导出」状态 Tag 使用现有设计系统的成功语义绿色，不新增硬编码状态配色
- AND 卡片以「归档案件」作为唯一推荐操作，并在更多菜单保留「打开案件」与「再次导出」
- AND 导出请求期间只使用当前页面按钮 loading 并禁止重复提交，不新增持久化「导出中」状态

#### Scenario: 导出完成后打开该案件最后导出文件夹
- WHEN 案件已至少成功完成一次导出
- THEN 案件卡片显示带悬浮提示和无障碍名称的文件夹图标按钮
- AND 点击后打开由该案件最后一条成功导出记录绑定的 Windows 文件夹
- AND 前端不向打开接口提交任意本机路径，后端不使用 shell 字符串执行路径
- AND 记录或目录不可用时显示可操作错误，不打开其他案件的目录

#### Scenario: 多案件并发导出保持目录绑定隔离
- WHEN 案件 A 的完成导出尚未结束时，用户又启动案件 B 的完成导出
- AND A 与 B 以任意顺序成功完成
- THEN 两个案件的导出 loading、成功标记和最后导出记录均按 `case_id` 独立收敛
- AND A 的图标只打开 A 的最后导出文件夹，B 的图标只打开 B 的最后导出文件夹
- AND 任一案件后完成不得以全局最后路径覆盖另一案件的目录绑定

#### Scenario: 统一删除仅删平台内产物
- WHEN 任意状态案件通过当前阶段入口执行“删除案件”（完成导出成功后为“归档案件”）并完成确认
...
#### Scenario: 同一页面的孤儿恢复扫描与图片上传交错
- WHEN 案件图片资产扫描在用户上传与绑定期间返回，且扫描结果包含正在处理的图片
- THEN 前端先等待当前上传绑定操作收敛，不得用扫描结果并发发起第二次图片绑定
- AND 当前操作的图片列表、成功或失败状态保留，不得被较早扫描响应覆盖或因页面自身竞态触发 `409 Conflict`
...
- **AND** 页面不渲染旧“笔录生成 / 审核编辑”页头、完整审核编辑表单、模式切换入口、独立保存操作和单独 Word 导出入口
...
#### Scenario: Word 内容预览与当前对话使用固定分栏

- **WHEN** 对话式引导视图存在可供核对的当前报告草稿或当前需要用户操作的事项
- **THEN** 桌面审核工作区铺满所在主工作区的可用宽度并使用等宽左右分栏，左侧展示 Word 内容预览，右侧展示当前对话，预览区和对话区分别滚动且当前操作与全局操作持续可达
- **AND** 页面不提供左右位置交换操作，视觉顺序、DOM 顺序和键盘顺序始终为 Word 内容预览在前、当前对话在后；既有浏览器本地分栏顺序偏好不得改变该固定顺序
- **AND** 当前对话区采用紧凑 Chat 形式展示獬豸助手身份、可替换的角色占位、当前推荐事项、结构化控件和主要操作；没有用户事项时显示等待后台处理、办理完成或现有恢复入口
- **AND** Word 内容预览只按文书结构渲染当前报告草稿中会影响 Word 正文或附件核对的最终内容，不展示报告来源确认、保存、编辑权限、后台归档、导出完成、异常恢复或字段办理完成等过程轨迹
- **AND** Word 内容预览中的可编辑业务信息 MUST 从当前报告草稿投影最终生效值；报告识别或系统带入且未修改的字段显示现值，用户填写或修改后的字段显示用户修改后的现值，不并列保留已经被覆盖的旧识别值
- **AND** 当现有 `FieldState` 或检材来源明确表明最终值由用户填写、修改或补充时，Word 内容预览 MUST 显示可见且可访问的来源标记；整项人工添加的检材仅在检材标题显示一次“人工添加”，其设备、类型、标识与提取情况不得重复标记；报告识别检材的单字段人工变更仅在对应字段显示“已修改”，其他直接填写字段继续显示“用户填写”；标记不得仅依赖颜色，也不得把报告识别或系统带入内容误标为用户输入
- **AND** 普通短文本以明确中文标签和值展示，多值使用符合业务习惯的分隔；案件简要情况、检查要求等长文本允许换行但不得以省略号隐藏最终内容；空值不得生成无意义占位
- **AND** 检查软件在 Word 内容预览中使用现有软件工具列表展示，不额外生成内容重复的“主取证软件”字段
- **AND** 检材和图片 MUST 按当前检材顺序结构化展示，每项使用“检材 N · 检材编号”作为主标签，并在右侧显示该检材现有图片数量与要求数量（例如 `2/2`）；检材设备信息、识别标识和用户填写的无法提取原因在对应检材下展示最终值，不得压缩成仅有总数的笼统描述
- **AND** Word 内容预览 MUST 直接把每项检材的最终字段值作为原位编辑入口，不得先点击检材级修改按钮、展开另一块完整检材编辑器、切换獬豸助手事项或进入完整审核编辑；设备名称、检材类型、是否可提取、无法提取原因、IMEI1、IMEI2、序列号和标题中的检材编号均可在原展示位置修改，图片仍由既有图片控件管理
- **AND** 预览内检材字段修改 MUST 直接更新现有 `introduction.evidence_list` 并沿用案件草稿自动保存、编辑租约、冲突恢复与检材完整性重置机制；字段区显示修改后自动保存、保存中、已保存或失败/冲突且输入保留的上下文状态，不提供额外保存按钮
- **AND** 除预览内检材编辑的上下文自动保存状态外，保存、来源、归档、导出和异常恢复等过程状态只在当前对话或对应操作区按需呈现，不得混入 Word 内容预览
- **AND** 每个当前事项先由獬豸助手使用完整、可执行的提示句说明下一步，例如“请输入文号”“请选择委托时间”或“请恢复草稿保存”，不得只显示“处理文号”等状态标签后直接呈现控件
- **AND** 当前任务说明、操作要求、系统处理状态、完成结果与恢复说明 MUST 由獬豸助手消息承载；例如检材图片的数量与排序提示、归档完成结果及返回工作台说明不得显示在“你的回复”内
- **AND** “你的回复”或“请选择操作”区域只承载用户实际需要填写、选择、上传、确认或触发的控件，以及紧邻控件且不可脱离的字段标签、格式约束和局部校验；不得重复助手已经说明的状态或操作指导
- **AND** 当前状态没有可供用户操作的控件时不得渲染空的回复区域；后台等待、办理完成等纯系统状态只由獬豸助手说明
- **AND** 当前事项在本次页面会话中完成后，当前对话区不累积用户完成回执或修改按钮，只展示当前推荐事项、结构化回复控件和必要的事项切换说明；不得保存逐字输入或新增持久化聊天记录
- **AND** 当前案件中已经完成的结构化事项统一进入“已填内容与待办”面板并保留返回对应既有控件的修改入口；步骤索引不进入案件草稿或服务端，刷新或重新进入时仍可从持久化步骤检查点和当前草稿事实恢复可回访的结构化事项
- **AND** 当前结构化事项完成并推进，或用户从“已填内容与待办”切换到另一个结构化事项后，当前案件均维护完整的步骤访问轨迹：回复框左下角只显示带 Tooltip 和明确可访问名称的左箭头图标按钮，用户可连续逐步后退直到最早记录；右下角只显示同样可辨识的右箭头图标按钮，用户可连续逐步前进直到最新进度。到达轨迹边界时不提供无效方向操作，同一位置不得并排显示两个右箭头。没有回复框时仍在助手提示之后保留适用入口。用户可重新查看或修改来源步骤的既有结构化控件，来源事项尚未完成时仍可返回，图片上传等高内容不得把入口推到回复区之后；步骤访问轨迹只记录用户实际填写、确认或主动选择的可操作事项，正常自动保存中的提示以及保存、租约、图片等临时系统恢复状态不得被记录为前后步骤，异常恢复操作仍通过当前操作集或全部待处理事项提供，恢复后不得残留在用户步骤之间；轨迹以 `caseId` 隔离的版本化浏览器本地检查点保存步骤参考和当前游标，不得回滚已保存草稿、把已完成事项重新计入待处理数量或持久化为新的业务事实
...
- **AND** 文本型当前事项继续把每次输入实时写入现有报告草稿和自动保存，但输入首个字符或后续 `change` 不得自动切换事项；用户可以按下非输入法组合态的 Enter，或点击输入区内提供明确 Tooltip 与可访问名称的圆形确认按钮来确认并推进，两种方式必须触发同一完成行为；多行文本的 Shift+Enter 继续用于换行，只读或导出忙碌时确认按钮必须禁用
- **AND** Word 内容预览随当前报告草稿更新时不得改变对话区滚动位置
- **AND** 窄屏和高倍浏览器缩放下分栏改为固定单列并取消不必要的嵌套滚动，Word 内容预览在上、当前对话在下；文字、结构化控件、键盘焦点与主要操作不得被裁切或遮挡

#### Scenario: Word 内容预览不成为新的业务事实

- **WHEN** 页面把当前报告草稿投影为 Word 内容预览
- **THEN** 预览只读取现有 `InspectionReport` 和 `FieldState` 的最终值，不得新增持久化对话消息、办理历史表或案件业务字段
- **AND** 刷新或重新进入案件时，预览根据当前报告草稿重建；除可重建的结构化步骤参考与游标外，不还原逐字对话、字段值副本或已失效的过程状态
- **AND** 预览不得展示内部路径、令牌、revision、Worker、堆栈、完整错误代码或技术日志

#### Scenario: 退出并再次打开后按人工填写状态选择入口

- **WHEN** 用户在獬豸助手的任一可操作结构化步骤安全退出或关闭页面，之后在同一浏览器再次打开同一案件
- **THEN** 仍有人工待办时，页面必须恢复到退出时的当前结构化步骤；例如退出时正在填写案件简要情况，再次进入仍展示案件简要情况的原结构化回复控件
- **AND** 全部人工内容填写完成且没有保存、租约、来源或图片恢复事项时，页面必须忽略退出前游标并固定打开“文号”的原结构化编辑步骤，不得先停留在完成端点；用户仍可沿重建轨迹逐步前进至当前完成或等待状态
- **AND** 仍有人工待办时，恢复后保留退出前已建立的步骤顺序与游标；全部人工内容填写完成时，则保留按当前事实重建的完整可回访顺序并将游标定位到“文号”，用户可连续返回任意更早的可回访步骤，也可再向前返回最新进度
- **AND** 案件正文与用户输入值继续由现有案件草稿和自动保存机制恢复；步骤检查点只保存版本、案件归属、可回访步骤参考和当前游标，不得存储逐字输入、报告字段值、技术错误详情或临时系统恢复状态
- **AND** 重新进入时必须使用当前报告草稿、`FieldState` 和动态操作集重建每个步骤；已失效、不可回访或属于保存/租约/图片等临时恢复事项的参考必须从轨迹中移除，不得用已过期的存储副本覆盖当前业务事实
- **AND** 存储不可用、检查点版本不兼容、数据无效、所属案件不匹配或原当前步骤已不可恢复时，页面必须忽略无效内容并回退到当前事实派生的第一个可处理步骤，不得阻止打开案件
- **AND** 不同案件的检查点必须严格隔离，不得把其他案件的当前步骤或导航轨迹恢复到本案件
...
- **AND** 用户在卡片中的操作继续调用现有字段更新、自动保存、图片绑定、来源重选、压缩决定和介质映射能力；最终输出只从案件工作台调用现有完成导出能力
...
#### Scenario: 獬豸助手是唯一审核编辑界面

- **WHEN** 用户打开具有可审核草稿的案件并处理任一待办或已填内容
- **THEN** 页面始终在獬豸助手内展示对应结构化控件，不提供“完整审核编辑”或“返回引导模式”入口
- **AND** 检材、检查人员、检查软件、检查步骤、普通文本、日期、照片和介质编号等可修改内容必须在獬豸助手或左侧 Word 内容预览的既有原位控件中可达
- **AND** 页面继续共用同一个案件会话、内存草稿、自动保存、编辑租约、图片资产和归档状态，不复制或转换业务数据
- **AND** 只读、租约失效、保存失败和图片尚未绑定的现有保护语义继续有效
...
- **AND** 用户选择不完整后在同一回复区直接展开快捷批量补充的具体编辑界面，不再要求点击中间入口，也不展示“逐项编辑”或切换到逐项编辑的按钮
- **AND** 当前检材清单和快捷批量补录状态中的每项检材均提供具有明确可访问名称的危险图标删除操作；用户确认删除后仅移除目标检材，保留其他检材及其顺序，并通过现有报告更新与自动保存链路将检材完整性恢复为未确认；用户取消时不得修改草稿
- **AND** 检材摘要与快捷批量补录区不得另设“确认完整”按钮；统一的“进入下一步”操作同时确认检材完整性并推进引导流程，且从历史回访该事项时行为一致
- **AND** 对话外壳不得复制检材编辑、草稿保存、字段来源或完整性确认逻辑

#### Scenario: Word 预览突出检材信息异常并支持逐项修改

- **WHEN** Word 内容预览展示报告识别或用户补充的检材
- **THEN** 设备和手机/平板类型均非空，且 IMEI1、IMEI2 均非空且两个值不同的检材统一收纳在一个默认折叠框内，展开后仍按当前检材顺序显示每项最终值
- **AND** 设备或类型任一为空（含纯空白）、IMEI1 或 IMEI2 任一缺失、两个 IMEI 均缺失，或者 IMEI1 与 IMEI2 的非空值相同的检材必须逐项单独展示，不得收进普通检材折叠框；设备与类型按当前预览展示值判断，补齐缺失内容且 IMEI 满足正常条件后自动恢复正常分组
- **AND** 每项检材继续展示检材编号、设备、手机/平板类型、IMEI1、IMEI2、序列号、提取情况和图片数量；缺失 IMEI 使用明确待核对状态，不得以隐藏字段表示完整
- **AND** 獬豸助手的当前检材摘要不得再提供逐项修改按钮或展开完整检材编辑器，只保留明确的检材摘要与删除操作；设备名称、检材类型、是否可提取、无法提取原因、IMEI1、IMEI2、序列号和检材编号统一在 Word 内容预览的对应字段原位修改
...
- **AND** 预览后的批次写入操作以带识别数量的“确认添加 N 项检材”文字按钮呈现，区别于统一的“进入下一步”；写入成功后检材完整性仍为未确认，直至用户进入下一步
- **AND** 快捷添加完成后仍展示当前检材清单，允许用户删除误添项或继续快捷批量添加

#### Scenario: 快捷补录统一解析排序并预览

- **WHEN** 用户在不完整检材回复区填写快捷补录内容
- **THEN** 输入框下方仅展示统一的“解析、排序并预览”圆形图标操作，提供明确 Tooltip 和可访问名称；不再单独展示解析、一键排序或确认完整按钮，检材完整性继续由“进入下一步”确认
- **AND** 用户点击统一按钮时，系统解析输入并使用报告识别阶段相同的多段数字自然升序规则排序合法预览，使 `JC1`、`JC2`、`JC10` 按该顺序排列；即时提示已识别并排序的数量，预览上方持续明确提示“预览已按检材编号自然升序排列，确认添加时将同步排序全部检材。”；确认添加前不写入案件，编辑输入后清除旧预览和排序成功提示；输入为空时仍可用此按钮排序已有检材并提示结果
...
- **THEN** 页面在下方当前对话区推荐一个当前事项，并始终提供带 Tooltip 和明确可访问名称的“返回案件列表”圆形图标；仍有人工待办或保存、租约、来源、图片恢复事项时，同时提供“已填内容与待办”圆形图标，全部人工内容填写完成且没有恢复事项后隐藏该图标，不保留零待办入口；不提供独立“查看已整理信息”、完整审核编辑或单独 Word 导出入口，当前回复或确认始终是唯一主操作
- **AND** 用户可以从“已填内容与待办”进入任一当前待处理事项，并回访当前草稿中全部可修改的已填内容及现值；文号、委托时间、检材完整性、检材照片、介质编号、刻录时间、检查人员、检查软件和检查步骤等内容必须可点击，点击后仍停留在獬豸助手并重新打开对应的原结构化回复控件，同时纳入本次会话的前进/后退轨迹。检材逐项详情继续留在 Word 内容预览中原位编辑；系统产物字段不进入“已填内容”。已上传完成的检材照片入口必须显示当前图片数量并允许继续检查、删除或重新上传，且不得重新计入待处理数量。正在处理或本次刚完成的同一字段不得重复展示
- **AND** “已填内容与待办”的数量角标只统计需要用户处理的事项，不把后台等待、可生成状态或已填写内容计入红色待办数量；系统处理状态继续在列表内明确展示
- **AND** “已填内容与待办”隐藏后，助手必须从当前草稿和用户字段状态重建全部可回访的人工结构化事项，并以当前完成或等待状态作为步骤轨迹的最新端点；用户可通过上一步逐项返回文号、委托时间、检材完整性、检材照片、介质编号、刻录时间等已处理控件，再通过下一步连续前进至最新状态。刷新、重新进入或浏览器本地检查点缺失时仍须可用；恢复事项或新人工待办出现时面板入口自动恢复
- **AND** 新出现的介质编号或恢复事项先进入全部待处理事项，在当前输入收敛后才成为当前对话，不突然替换用户正在编辑的内容
- **AND** 除“委托时间→案件简要情况”作为绪论字段的相邻顺序，以及“上传检材照片→填写介质编号”作为附件事项的明确先后关系外，助手不得为文号、检材等其余字段采用额外固定题序；已由报告、默认设置或用户操作满足的事项直接从操作集中消失，但报告解析出的案件简要情况仍须在本次页面会话中以预填多行输入框提供一次人工核对和修改，确认后不重复询问
- **AND** 用户在獬豸助手的检材照片事项中批量上传并补齐所需图片后，当前图片步骤必须继续展示已导入图片供用户核对，不得因图片待办实时消失而自动切换到下一事项；只有用户明确点击“进入下一步”后才推进引导流程。图片上传或绑定异常时，错误提示与可继续上传的图片控件必须合并在同一步骤，不得同时生成“图片恢复”和“检材照片”两个页面
- **AND** 压缩时机属于可自由选择的现有操作，选择稍后压缩不得被重新解释为必须立即压缩，也不得通过重复提问把用户锁回立即压缩路径

#### Scenario: 正常办理在检材照片之后填写介质编号

- **WHEN** 案件没有保存、租约、来源或图片恢复事项，且处于可选择压缩时机的审核状态
- **THEN** 獬豸助手首先推荐现有“选择压缩时机”操作，不得先推荐文号、委托时间或其他普通审核字段
- **AND** 用户完成压缩时机选择后，助手按现有动态待办继续办理普通审核字段；检材照片与介质编号同时待处理时，必须先推荐上传检材照片，再推荐填写介质编号
- **AND** 图片上传完成后仍停留在检材照片步骤供用户核对，只有用户明确进入下一步才继续到介质编号或后续事项
- **AND** 检材照片已经补齐时不重复要求上传；介质编号已经满足现有校验时也不重复询问，且不得复制或改变字段校验规则
- **AND** 保存失败、编辑租约失效、来源不可用或图片未安全绑定等会阻止安全继续的恢复事项仍优先于该正常办理顺序
- **AND** 用户选择稍后压缩后可以继续处理其他审核事项，并保持检材照片先于介质编号；再次立即压缩入口保留在全部待处理事项中，不得持续占用当前推荐操作

#### Scenario: 案件简要情况紧随委托时间填写

- **WHEN** 獬豸助手的动态操作集中同时存在委托时间、案件简要情况和其他后续审核事项
- **THEN** 案件简要情况必须排列在委托时间之后、其他后续审核事项之前，不得被追加到全部事项末尾
- **AND** 报告已经解析出的案件简要情况仍使用预填多行输入框供人工核对，既有压缩时机、介质编号和恢复事项优先级保持不变
- **AND** 案件简要情况末尾存在回车、空格或制表符时，獬豸助手必须显示明确的清理警告；用户删除尾部空白后警告随即消失
...
- **AND** 图片上传或绑定失败继续遵守既有离页保护并提供现有图片控件恢复入口，不得静默声称图片已经安全绑定或进入最终输出
- **AND** 恢复事项的完成仍由既有保存、租约、图片和导出事实重新派生，不新增对话层完成标记，也不改变现有 Export Gate 或工作台完成导出合同
- **AND** 用户正在编辑其他卡片时，新出现的恢复事项先进入历史区和全部待处理事项；除非当前操作已经不可写或继续操作会丢失数据，不得突然卸载或替换用户正在输入的控件

#### Scenario: 已填内容与待办保持可预测焦点

- **WHEN** 用户在獬豸助手中打开“已填内容与待办”或选择其中的具体事项
- **THEN** 触发器通过稳定标识与其展开区域建立可访问关系，展开、收起、键盘顺序和现有快捷键在窄屏及浏览器缩放下保持可用
- **AND** 选择事项后焦点移动到当前对话的具名标题或对应结构化控件，不得遗留在已关闭的面板中
- **AND** 用户展开“已填内容与待办”后，目标面板必须立即进入当前单一滚动视口的可见范围并获得可预测的键盘焦点，不要求用户继续向下滚动才能看到内容
...
- **THEN** 助手必须把该结果展示为“草稿已保存”的专用完成态，并以“压缩已设为稍后处理”说明当前归档选择；不得把它伪装成仍需完成的普通当前步骤
- **AND** 完成态内的唯一主操作必须是带可见文字的“返回案件工作台”，直接复用现有安全退出入口；“现在压缩”作为次操作继续调用现有立即压缩入口，“返回上一步修改”作为低强调操作保留既有步骤轨迹能力
- **AND** 完成态不得把孤立的图标后退按钮作为唯一显著操作；所有可见标签、可访问名称和说明必须使用一致的“案件工作台”术语，并准确说明动作结果
- **WHEN** 工作台完成导出成功或后台归档仍在处理
- **THEN** 助手分别使用“已完成导出”或“后台归档处理中”的准确状态，不把最终输出和中间状态合并成单一“完成”终态

#### Scenario: 最终生成继续由现有保存与导出门控裁决

- **WHEN** 引导视图根据现有派生结果没有发现当前可处理的待核对事项
- **THEN** 獬豸助手展示“当前审核已完成”、返回案件工作台完成导出的提示和唯一主操作“保存并退出”，不得展示归档完成/盘号映射面板或单独 Word 生成按钮，也不得把对话层状态作为正式导出许可
- **AND** 用户点击“保存并退出”时，页面继续等待现有图片绑定和自动保存收敛；仅在保存成功后返回案件工作台，保存失败或冲突时留在当前页面并提供既有恢复操作
- **AND** 案件审核页面不提供任何导出入口；用户只能返回案件工作台，通过对应案件卡片执行完成导出
- **AND** 现有 Export Gate、模板校验、归档校验或最新 revision 返回阻断时，页面以可操作的用户语言展示原有错误并返回对应现有操作，不绕过、放宽或复制门控
- **AND** 工作台完成导出的图片、归档和介质要求继续遵守现有合同，入口收口不得改变后端导出能力或工作台导出行为
...
生产后端 MUST 只监听loopback固定端口40000，前端与API MUST 同源，API及受保护下载 MUST 拒绝没有本次桌面启动授权的请求；开发模式 MUST 保持现有Vite代理工作流。

#### Scenario: 固定端口启动
- **WHEN** 用户启动便携 EXE
- **THEN** 网页与 API 共用 http://127.0.0.1:40000，首次打开与托盘重新打开均使用该端口
- **AND** 端口不可用时明确提示固定端口40000不可用并停止启动，不得随机选择其他端口
- **AND** 启动器必须校验就绪握手端口与请求端口一致
...
### Requirement: 本机来源目录无需白名单授权

系统 MUST 允许首次案件登记、原生目录选择、来源替换和旧版目录解析直接使用有效本机报告目录，不再读取来源白名单配置或浏览器来源校验偏好，不再提供来源校验开关。来源请求合同 MUST 移除 `source_authorization_enabled` 和 `directory_grant_token`；工作台 JSON 请求对已移除字段按既有严格 DTO 返回 422，旧版表单中的废弃额外字段不得改变行为。

系统 MUST 保留基础路径安全、报告结构、输入输出隔离、来源身份和不透明上下文检查。用于导出目标目录的一次性授权令牌 MUST 保留有效期、精确路径绑定和防重放检查，不与来源目录功能一并删除。

#### Scenario: 任意有效本机目录可登记和替换

- **WHEN** 用户选择白名单外的有效本机报告目录，或替换既有来源
- **THEN** 系统完成来源登记，不要求来源授权令牌
- **AND** 旧浏览器校验偏好及旧来源白名单环境配置不影响行为

#### Scenario: 基础路径和报告结构检查保留

- **WHEN** 用户提交相对、网络、设备、链接、与输出区域重叠的路径或不支持的报告目录
- **THEN** 系统返回对应稳定错误，不建立来源或启动解析，不回显完整路径

#### Scenario: 移除工作台校验和手动刷新入口

- **WHEN** 用户进入案件工作台
- **THEN** 不显示来源目录校验开关或手动刷新按钮
- **AND** 上传目录、任务自动更新、错误重试和案件卡片操作继续可用
