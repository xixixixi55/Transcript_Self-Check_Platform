...
系统 MUST 为每个最终归档卷生成一页 `Attachment3Plan`；只有第一页显示“附件3”，后续页结构一致但不显示该标题。每页 MUST 使用 manifest 中对应的归档文件、实际大小、MD5、光盘编号和刻录日期；不得重新从目录或报告原始字段推导这些值。正文检查结果在同一导出中 MUST 使用该 manifest 的全部有序分卷文件名、实际大小、MD5 和光盘编号，不得退回报告中单个旧分卷字段。
