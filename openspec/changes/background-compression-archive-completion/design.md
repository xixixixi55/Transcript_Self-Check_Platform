...
**理由**：统一导出会先把全部 RAR 复制到用户目录的同卷 staging，再由 HashMyFiles 读取这些副本。固定 30 分钟外层请求比 HashMyFiles 既有最长执行预算更短，也无法覆盖 135 GB 容量上限下机械盘的两遍完整 I/O，会造成后端仍在正常工作而前端先报告失败。完整性校验原先按 50 MB/s 且没有固定余量估算，对老旧、繁忙或受实时防护影响的机械盘同样偏紧。
