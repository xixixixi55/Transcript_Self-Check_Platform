...
- WHEN 工作人员点击「上传报告目录」并打开 Windows 原生目录选择器
...
- WHEN 图片引用已经保存，工作人员随后人工新增、删除、改号或调整检材顺序
...
- AND 盘号后填或盘号修改不破坏 Manifest 复用（盘号从复用指纹中解耦）；新的非统一导出内容访问仍按 Manifest 绑定算法重新验证实际 part 的存在性、大小和完整摘要
...
- THEN `AttachmentPlan` 按 Manifest 中每个实际 part 生成一行数据，列结构固定为：序号、电子数据、来源、提取方式、案件所选文件哈希值
...
- AND Word 与 RAR 同批原子发布；再次导出时移除历史 `hash-verification.png/html`，发布失败恢复上一版完整导出
...
- WHEN 工作人员在审核编辑界面的附件图片区域点击新增的「批量导入图片」按钮，并一次选择多张文件名含数字的图片
...
- WHEN 当前有 101 个检材且工作人员一次选择 202 张符合要求的图片
...
- WHEN 工作人员发起单独 Word 导出，而当前图片缺失、数量不匹配、映射无效、尚未绑定、上传或绑定持续未完成，或无法读取
...
- WHEN 案件的归档计划已完成全部分卷盘号映射，但草稿兼容字段 `attachments.disc_number` 为空，工作人员在审核页单独点击导出 Word
...
- WHEN 后端对全部实际 RAR 分卷执行 `rar t` 完整性校验或所选文件哈希校验
...
- THEN 前端必须先等待当前上传绑定操作收敛，不得用扫描结果并发发起第二次图片绑定
- AND 当前操作的图片列表、成功或失败状态必须保留，不得被较早扫描响应覆盖或因页面自身竞态触发 `409 Conflict`
...
- AND 写入探针遇到权限拒绝、只读、离线盘或其他 I/O 错误时必须在有限尝试内返回，不得因临时文件名重试挂起启动或持续占用 CPU
