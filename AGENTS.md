...
验证输出先给通过/失败汇总和按类型计数；失败默认不展开逐条日志，追加 `--details` 后再下钻具体日志。
运行 pytest 默认使用安静模式和短 traceback（`-q --tb=short`）：先只读取退出码、最终统计和失败数量；通过时不逐条读取通过用例，失败时只下钻失败用例及其 traceback。修复后先重跑失败用例，再按需运行受影响模块；除非明确需要，不使用 `-v` 或逐条输出。
