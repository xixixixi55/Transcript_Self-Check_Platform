...
当前内置模板的 A4 左右页边距 MUST 对称；正文直接段落的左右排版边界 MUST 围绕页面中心平衡，附件一固定表格 MUST 相对页面水平居中。修正版 MUST 发布为新版本，既有案件明确引用的旧内置版本继续从历史资产重导出；启动迁移只更新仍指向旧内置默认版本的共享默认值，不改写案件引用或用户自定义默认模板。
