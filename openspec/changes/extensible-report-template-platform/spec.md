...
适配结果 MUST 将案件信息、报告完成时间、设备材料、标识符、软件版本候选和字段来源映射到 `CanonicalInspectionCase`/`FieldProvenance`，再投影现有 `InspectionReport` 供当前审核和导出链路使用。案件创建时间与报告完成时间是案件级时间边界候选，设备取证起止时间只保留来源；主取证软件名称或版本不能可靠唯一绑定时、检材类型不能由报告明确确认时，系统 MUST 允许审核修正并在确认前阻止正式导出。

#### Scenario: 当前结构的单检材平航报告正常解析

- **WHEN** 用户选择一份命中 `pinghang-mobile-multipath-v1` 全部结构签名的单检材报告目录
- **THEN** 系统登记来源并生成包含案件、一个材料、设备标识符、软件版本候选、检查时间候选和 provenance 的审核草稿
