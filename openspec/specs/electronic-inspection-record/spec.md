...
内置模板以不可变版本保存。当前 `electronic-inspection-record@1.0.1` 不包含批注结构或附件二示例媒体，但保留附件二空白定位段落、VML、分页、表格和动态图片渲染合同；历史 `1.0.0` 资产继续供既有案件重导出，但不进入新案件可选列表，且不能再设为默认或删除。启动迁移将仍指向历史只读 `1.0.0` 的内置默认模板迁移到 `1.0.1`，不改写已有案件引用或用户选择的其他默认模板。
...
- THEN 新案件默认引用 `electronic-inspection-record@1.0.1`
- AND 已明确引用 `1.0.0` 的案件继续使用历史资产，案件模板引用不变
- AND 新案件不能选择 `1.0.0`，管理接口不能将其重新设为默认或删除
