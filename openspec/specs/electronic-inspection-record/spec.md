...
- **AND** 当前结构化事项完成并推进，或用户从“已填内容与待办”切换到另一个结构化事项后，本次页面会话均在助手提示与回复控件之间提供“返回上一步”入口，允许用户重新查看或修改来源步骤的既有结构化控件，并提供返回最新当前步骤的入口；来源事项尚未完成时仍可返回，图片上传等高内容不得把入口推到回复区之后；步骤导航不得回滚已保存草稿、把已完成事项重新计入待处理数量或持久化为新的业务事实
...
- **THEN** 页面可以展示“笔录已准备完成”和单独 Word 生成入口，但不得把对话层状态作为正式导出许可
- **AND** 案件内部的引导视图与完整审核编辑均不提供统一导出入口；用户只能返回案件工作台，通过对应案件卡片执行统一导出
- **AND** 用户在案件内部点击单独 Word 生成后继续等待现有图片绑定和自动保存收敛，并调用现有 Word 导出流程
