...
内置模板以不可变版本保存。当前 `electronic-inspection-record@1.0.3` 不包含批注结构或附件二示例媒体，保留附件二空白定位段落、VML、分页、表格和动态图片渲染合同，并使用对称 A4 左右页边距、平衡的正文左右排版边界、相对页面居中的附件一固定表格与粗横线、无制表符伪居中的可见主标题，以及一级略突出、同级对齐的结构标题层级。历史 `1.0.0`、`1.0.1`、`1.0.2` 作为已退役资产继续供既有案件重导出，但不进入模板管理列表或新案件可选列表，且不能再设为默认或作为编辑源。启动迁移将仍指向历史只读内置版本的默认模板迁移到 `1.0.3`，不改写已有案件引用或用户选择的其他默认模板。
