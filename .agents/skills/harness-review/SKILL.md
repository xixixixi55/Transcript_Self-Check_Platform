...
读取项目根目录对应工具目录下的 `commands/harness/review.md` 获取详细执行协议，按其步骤执行 `/harness:review` 的完整流程。

**快速参考**（完整步骤见命令文件）：
1. 维度一：完整性 — 任务完成度 + spec 需求覆盖率
2. 维度二：正确性 — 逐条对照 spec 场景，检查代码与风险相称的验证证据
3. 维度三：一致性 — design.md 决策遵循 + Harness 架构约束
4. 生成结构化报告（CRITICAL / WARNING / SUGGESTION）
5. 输出归档建议
6. Level 2 必须对照其精简 delta spec；不以“只有 tasks.md”跳过行为要求检查，且不因此增加 proposal/design/verify/review 强制要求
