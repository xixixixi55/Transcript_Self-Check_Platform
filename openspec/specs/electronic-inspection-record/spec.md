...
- **AND** 保存、来源、归档、导出和异常恢复等过程状态只在当前对话或对应操作区按需呈现，不得混入 Word 内容预览
...
- **AND** 用户可从每项检材进入獬豸助手的原检材结构化控件，并修改设备名称、检材类型、是否可提取、无法提取原因、IMEI1、IMEI2、序列号和检材编号；修改继续使用现有草稿更新、字段来源和自动保存链路
