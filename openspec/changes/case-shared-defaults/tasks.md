...
- 初始化优先级为：系统强制默认值 → 无共享值时的 Parser 非空回退值 → 非空共享默认值 → 当前案件用户手工修改。
- Parser 的空字符串、空数组和缺失字段不得擦除已应用的共享默认值；已有共享值时 Parser 同字段不覆盖共享预填，且 Parser 值不自动更新共享默认值。
...
- [ ] 第一个案件修改六个字段并成功保存后，第二个由后端创建的新案件使用最新非空合法共享默认值预填。
- [ ] 第二个案件成功保存新值后，第三个新案件使用第二个案件的最新成功保存值。
- [ ] 共享默认值更新只影响以后创建的新案件，既有案件的草稿、revision、来源、图片和状态保持不变。
- [ ] 空字符串、纯空格、校验失败和草稿保存失败均不得覆盖已有共享默认值。
- [ ] 解析过程中的临时值、尚未成功保存的输入和前端失败重试状态不得更新共享默认值。
- [ ] 草稿保存成功而共享默认值更新失败时，草稿保持成功；接口返回稳定的默认值失败诊断，下一次成功保存可再次尝试。
- [ ] 旧案件只修改图片或案件名称时，不覆盖已有共享默认值；案件切换不会静默提交六字段。
- [ ] 自动保存重试不扩大默认值 patch；Parser 自动填充值未经用户明确修改不更新共享默认值。
- [ ] 新建空白案件/初始草稿时即可获得共享默认值；Parser 缺失地点保留共享值，Parser 非空地点只覆盖当前案件；重新解析既有案件不被共享值改写。
- [ ] 草稿成功而默认值失败时，接口、Hook 和 UI 明确表现为“草稿已保存，共享默认值更新失败”，不触发整体草稿重复保存。
- [ ] 服务端重启或 repository 重建后仍读取同一部署实例的默认值；客户端不能伪造 `deployment_instance_id`。
- [ ] 未知字段请求整体拒绝且不发生部分写入；合法六字段采用稀疏增量合并，空值不清空旧值。
- [ ] 光盘前缀不会直接成为完整编号；不会继承上一案件日期/案件号/序号/后缀，不同案件不会因同一前缀生成重复完整编号，Word 使用当前案件完整编号。
- [ ] 只修改一个共享字段时，其他五个默认值保持不变。
- [ ] 并发保存以最后一次成功保存的有效值为准；revision 冲突不会覆盖较新的共享默认值。
- [ ] 后端重启、浏览器刷新和关闭重开浏览器后仍能读取同一部署实例的共享默认值。
- [ ] API 只接受六个白名单字段；未知字段、案件生命周期、路径、PID、revision、内部状态、Manifest、RAR 和运行时字段统一拒绝且不部分写入。
- [ ] 文号保存完整输入值，不自动递增、不拆分年份/单位/编号；新案件只预填最近一次成功保存值。
- [ ] 光盘默认值只保存前缀；新案件不会复制其他案件完整编号、序号或后缀，不新增自动递增逻辑，并继续使用既有光盘解析/生成逻辑。
- [ ] 检查人员保持现有数组或字符串格式、顺序和 Word/DTO 分隔规则，不改变公共填充格式。
- [ ] 检查硬件设备只来自表单字段，不记忆来源设备、被检查对象或附件设备信息。
- [ ] 除六字段外，案件名称、被检查对象、时间、来源目录、图片附件、数据摘要、案件/草稿/来源/归档状态、RAR、Manifest、路径、PID 和进程字段均不会进入共享默认值。
- [ ] 数据摘要仍使用现有固定业务默认值 `即时通讯、手机信息`，不受共享默认值接口影响。
- [ ] Legacy Parser、Word 预览、Word 导出、VML、分页、Manifest 和归档合同回归通过。
...
- [x] 将六字段应用接入 `CaseDraftService._initialize_draft` 的后端新案件初始化链：系统强制默认值 → Parser 回退值 → 非空共享默认值 → 用户手工修改；不修改已存在案件。
...
- [x] 新案件初始化改为非空共享默认值优先于 Parser 值；共享默认值为空时回退到 Parser，既有案件和用户手工修改不受影响。
...
统一保存修复、后端全量、完整 Harness 和人工验收均已通过；结果不代表 Phase 1D Production Review 或 OpenSpec 归档通过。未 commit、未 push、未归档。
