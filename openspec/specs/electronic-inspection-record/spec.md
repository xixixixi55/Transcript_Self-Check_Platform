...
#### Scenario: Word 预览突出 IMEI 异常并支持逐项修改

- **WHEN** Word 内容预览展示报告识别或用户补充的检材
- **THEN** IMEI1、IMEI2 均非空且两个值不同的检材统一收纳在一个默认折叠框内，展开后仍按当前检材顺序显示每项最终值
- **AND** IMEI1 或 IMEI2 任一缺失、两个 IMEI 均缺失，或者 IMEI1 与 IMEI2 的非空值相同的检材必须逐项单独展示，不得收进普通检材折叠框
