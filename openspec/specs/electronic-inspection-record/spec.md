...
- **AND** 本次页面会话中已经完成的结构化事项统一进入“已填内容与待办”面板并保留返回对应既有控件的修改入口；该会话级修改索引不进入案件草稿或服务端，刷新或重新进入仍可通过面板内的完整审核编辑入口修改其他既有内容
- **AND** 当前结构化事项完成并推进，或用户从“已填内容与待办”切换到另一个结构化事项后，本次案件页面会话均维护完整的步骤访问轨迹：回复框左下角只显示带 Tooltip 和明确可访问名称的左箭头图标按钮，用户可连续逐步后退直到最早记录；右下角只显示同样可辨识的右箭头图标按钮，用户可连续逐步前进直到最新进度。到达轨迹边界时不提供无效方向操作，同一位置不得并排显示两个右箭头。没有回复框时仍在助手提示之后保留适用入口。用户可重新查看或修改来源步骤的既有结构化控件，来源事项尚未完成时仍可返回，图片上传等高内容不得把入口推到回复区之后；步骤访问轨迹只记录用户实际填写、确认或主动选择的可操作事项，正常自动保存中的提示以及保存、租约、图片等临时系统恢复状态不得被记录为前后步骤，异常恢复操作仍通过当前操作集或全部待处理事项提供，恢复后不得残留在用户步骤之间；轨迹只存在于当前页面会话，不得回滚已保存草稿、把已完成事项重新计入待处理数量或持久化为新的业务事实
...
- **AND** 刷新或重新进入案件时，预览根据当前报告草稿重建，不承诺还原逐字对话、操作顺序或已失效的过程状态
