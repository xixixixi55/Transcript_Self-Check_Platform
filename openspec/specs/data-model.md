...
SQLite 持久意图/发布记录是工作台直出归档的唯一权威发布事实。数据库参与的流程不读取、创建、锁定或重写全局 JSON Manifest 索引；该索引只保留给无数据库旧流程，并继续以失败关闭方式维护。公共任务/结果投影只暴露获准的不透明产物元数据，绝不暴露绑定发布的代次、所有者、栅栏或文件系统定位符。
...
`SharedDefaults` 由后端持久化，并以当前本地操作员所在部署为范围；该范围不提供也不宣称多用户隔离。可编辑业务值仅限文号模板、检查地点、检查方法、硬件设备、数据摘要、检查要求、有序检查人员快照及 `md5 | sha1 | sha256` 哈希算法。委托单位前缀已废止：旧持久值可被安全读取但不通过 API 返回、不参与新案件初始化，并在后续持久化时丢弃；写入请求携带该字段时按未知字段拒绝。为兼容持久部署，Legacy 附件一提取方法值仍可读取，但集中设置页不显示或提交，新案件初始化也不使用。旧的完整文号和盘号前缀继续作为 API 兼容持久值，但集中设置页不编辑它们。
