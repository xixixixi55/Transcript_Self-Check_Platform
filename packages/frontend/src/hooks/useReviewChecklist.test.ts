...
describe('getReviewPendingItems', () => {
  ...
  it('缺少显式状态时默认可提取且不要求填写无法提取原因', () => {
    const serialOnly = {
      ...
      material_type_source: 'user' as const, serial_number: 'SYNTHETIC-SERIAL', imei1: '', imei2: '',
    }
  ...
  it('按归档介质区分 GP 与 YP 编号校验', () => {
    ...
    expect(getReviewPendingItems(withNumber('YP2026082002-01'), undefined, 'hard_drive')
    ...
    expect(getReviewPendingItems(withNumber('YP20260820-01'), undefined, 'hard_drive')
      .some(item => item.fieldLabel === '硬盘编号' && item.kind === 'validation')).toBe(false)
    expect(getReviewPendingItems(withNumber('GP2026082002-01'), undefined, 'hard_drive')
      .some(item => item.reason.includes('YPyyyyMMdd[XX]-序号'))).toBe(true)
    expect(getReviewPendingItems(withNumber('YP2026082002-01'), undefined, 'optical_disc')
      .some(item => item.reason.includes('GPyyyyMMdd[XX]-序号'))).toBe(true)
    expect(getReviewPendingItems(withNumber('YP2026082002-01'), undefined, null)
      .some(item => item.kind === 'validation' && item.targetId === REVIEW_TARGET_IDS.discNumber)).toBe(false)
    expect(getReviewPendingItems(withNumber('GP20260820-01'), undefined, 'optical_disc')
      .some(item => item.kind === 'validation' && item.targetId === REVIEW_TARGET_IDS.discNumber)).toBe(false)
    expect(getReviewPendingItems(withNumber('AB2026082002-01'), undefined, null)
      .some(item => item.reason.includes('GPyyyyMMdd[XX]-序号 或 YPyyyyMMdd[XX]-序号'))).toBe(true)
  })
})
