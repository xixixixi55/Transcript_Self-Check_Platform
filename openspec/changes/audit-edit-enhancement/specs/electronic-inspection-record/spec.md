...
## ADDED: CAP-ENTRUST-UNIT-PREFIX — 委托单位共享前缀

### REQ-ENTRUST-UNIT-PREFIX-001: 前缀可共享、可清空并与报告识别单位组合

**Scenario: 设置委托单位前缀**
- GIVEN 报告识别出的委托单位为“府前派出所”
- WHEN 用户将委托单位前缀设置为“宜都公安分局”并成功保存
- THEN 当前案件草稿保留独立的前缀和报告识别单位
- AND 共享默认值更新为“宜都公安分局”
- AND 后续新案件使用该前缀预填

**Scenario: Word 直接拼接前缀和识别单位**
- GIVEN 委托单位前缀为“宜都公安分局”
- AND 报告识别出的委托单位为“府前派出所”
- WHEN 系统生成 Word
- THEN 委托单位显示为“宜都公安分局府前派出所”
- AND 两部分之间不增加空格或分隔符

**Scenario: 清空委托单位前缀**
- GIVEN 共享默认值中已有委托单位前缀
- WHEN 用户将前缀清空并成功保存
- THEN 共享默认值中的前缀更新为空
- AND Word 仅显示报告识别出的委托单位
