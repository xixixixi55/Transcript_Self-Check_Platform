...
- AND “此前已处理”显示案件简要情况及当前值，点击后仍在獬豸助手中打开原文本核对控件
...
- WHEN 任意状态案件通过当前阶段入口执行「删除案件」（统一导出成功后为「归档案件」）并完成确认
...
- THEN 实际可见的推荐操作依次为无、重试解析、打开案件、打开案件、打开案件、统一导出、归档案件
...
- **THEN** 页面在下方当前对话区推荐一个当前事项，并始终提供带 Tooltip 和明确可访问名称的“返回案件列表”圆形图标；仍有人工待办或保存、租约、来源、图片恢复事项时，同时提供“已填内容与待办”圆形图标，全部人工内容填写完成且没有恢复事项后隐藏该图标，不保留零待办入口；不提供独立“查看已整理信息”入口，完整审核编辑作为面板可见期间的兜底修改入口，当前回复或确认始终是唯一主操作
- **AND** 用户可以从“已填内容与待办”进入任一当前待处理事项、返回本次会话已完成的既有结构化控件，并查看由当前草稿事实恢复、且确由用户通过獬豸助手直接填写或确认的此前处理字段及现值；文号、委托时间、检材完整性、检材照片、介质编号、刻录时间等此前处理内容必须可点击，点击后仍停留在獬豸助手并重新打开对应的原结构化回复控件，同时纳入本次会话的前进/后退轨迹，不得跳转到完整审核编辑。检材在“此前已处理”中只显示单一“检材完整性”事项；设备、类型、IMEI、序列号、提取情况、检材编号等逐项详情即使由用户修改也不得成为独立已处理事项，继续留在 Word 内容预览中编辑。已上传完成的检材照片入口必须显示当前图片数量并允许继续检查、删除或重新上传，且不得重新计入待处理数量。检查步骤、系统产物及其他由识别、模板或规则生成而非用户直接办理的内容不得进入“此前已处理”。正在处理或本次刚完成的同一字段不得重复展示。用户仍可通过“修改其他已填内容”进入完整审核编辑，不被强制锁定在固定的一题一页顺序中
...
- **AND** 案件简要情况末尾存在回车、空格或制表符时，獬豸助手必须显示与完整审核编辑一致的清理警告；用户删除尾部空白后警告随即消失
...
- **AND** “已填内容与待办”的触发器通过稳定标识与其展开区域建立可访问关系，展开、收起、键盘顺序和现有快捷键在窄屏及浏览器缩放下保持可用
...
- **THEN** 獬豸助手展示“当前审核已完成”、返回案件工作台统一导出的提示和唯一主操作“保存并退出”，不得展示归档完成/盘号映射面板或单独 Word 生成按钮，也不得把对话层状态作为正式导出许可
- **AND** 用户点击“保存并退出”时，页面继续等待现有图片绑定和自动保存收敛；仅在保存成功后返回案件工作台，保存失败或冲突时留在当前页面并提供既有恢复操作
- **AND** 案件内部的引导视图与完整审核编辑均不提供统一导出入口；用户只能返回案件工作台，通过对应案件卡片执行统一导出
- **AND** 单独 Word 入口只保留在完整审核编辑中；用户从该入口生成时继续等待现有图片绑定和自动保存收敛，并调用现有 Word 导出流程
- **AND** 现有 Export Gate、模板校验、归档校验或最新 revision 返回阻断时，页面以可操作的用户语言展示原有错误并返回对应现有操作，不绕过、放宽或复制门控
- **AND** 工作台统一导出的图片、归档和介质要求继续遵守现有合同，入口收口不得改变后端导出能力或工作台导出行为
