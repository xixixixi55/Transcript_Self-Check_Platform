...
- AND 首次迁移先完整暂存并排他发布 RAR，登记最终位置成功后清理工作区原件；写入或登记失败时回滚本次文件并保留原件
...
系统 SHALL 在统一导出成功后将 RAR 的唯一最终位置持久化到案件对应的 Manifest，清除工作区原件，重复导出使用同一份校验通过的外部 RAR。临时迁移失败 SHALL 保留原件；外部同名不相关文件 SHALL 不被覆盖；案件删除和保留策略 SHALL 不删除报告目录或其中最终导出文件。

#### Scenario: 成功导出并重启
- **WHEN** 统一导出成功并重启服务后再次导出
- **THEN** 读取并校验报告上级目录中的同一份 RAR，不再次复制或压缩，只更新 Word

#### Scenario: 迁移失败或冲突
- **WHEN** 目标同名 RAR 已存在且非本次登记产物，或写入、登记失败
- **THEN** 操作失败保留工作区原件与已有目标文件，允许重试

#### Scenario: 删除已导出案件
- **WHEN** 用户删除已导出案件
- **THEN** 清理应用受控数据，保留外部报告、最终 RAR 和 Word

| 用途 | 路径 |
|------|------|
| 报告解析结果 | 不持久化；同一规范化来源仅共享当前在途任务 |
| 归档文件 | 生成校验期间位于 `output/compressed/`；统一导出成功后仅保留报告文件夹上一级目录的 RAR（本地，不得进入 Git） |
