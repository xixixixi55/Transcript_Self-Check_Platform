...
- **THEN** 页面在下方当前对话区推荐一个当前事项，同时以带 Tooltip 和明确可访问名称的圆形图标工具栏提供“已填内容与待办”“查看已整理信息”和“返回案件列表”；完整审核编辑作为“已填内容与待办”面板内的兜底修改入口，当前回复或确认始终是唯一主操作
...
- **AND** “已填内容与待办”和“查看已整理信息”的触发器通过稳定标识与其展开区域建立可访问关系，展开、收起、键盘顺序和现有快捷键在窄屏及浏览器缩放下保持可用
- **AND** 用户展开“已填内容与待办”或“查看已整理信息”后，目标面板必须立即进入当前单一滚动视口的可见范围并获得可预测的键盘焦点，不要求用户继续向下滚动才能看到内容
