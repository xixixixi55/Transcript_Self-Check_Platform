...
- **AND** 卡片直接显示“删除”按钮，标准“删除”操作不出现在“更多操作”菜单中
- **WHEN** 用户点击“确认”
...
#### Scenario: 可变长度盘号前缀生成一致的 Manifest 日期与序列
- WHEN 用户以 1 至 20 个中英文字符作为首个光盘编号前缀，并以合法日期和序号启动归档
- THEN 系统必须从同一次结构化盘号解析结果取得日期并生成全部连续光盘编号，不得按固定字符位置截取日期
- AND 每个实际 RAR 的 `disc_number`、`disc_date` 与发布前复核使用同一序列事实源，多分卷归档必须完成发布
- AND 非法日期、非法编号或超出前缀长度限制仍按既有稳定错误拒绝

### Requirement: REQ-031: 归档完成与已导出状态机

归档完成态、导出路径提示、已导出标记与统一删除。
...
- WHEN 统一导出写入用户路径成功
- THEN 案件卡片标记为「已导出」
- AND 导出成功后仍可再次导出（可重复），卡片继续提供统一的「删除」按钮

#### Scenario: 统一删除仅删平台内产物
- WHEN 任意状态案件执行卡片上的「删除」并完成确认
