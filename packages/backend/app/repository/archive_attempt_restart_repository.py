...
def interrupt_owned_claim(
    ...
    """Atomically settle one local claim after a bounded runtime stop."""
    ...
    with database.transaction() as connection:
        ...
        if task is None or int(task["revision"]) != task_revision or task["status"] not in {"running", "cancelling"}:
            return "ignored"
        binding = _binding(task["process_binding_json"])
        if binding.get("process_tree_id") != owner_token or binding.get("staging_asset_id") != attempt_id:
            return "ignored"
        attempt = connection.execute(
            "SELECT * FROM archive_attempts WHERE attempt_id = ? AND case_id = ?",
            (attempt_id, task["case_id"]),
        ).fetchone()
        if attempt is None:
            return "ignored"
        ...
        if attempt["status"] in {"accepted", "running"}:
            _interrupt_attempt_in_transaction(connection, attempt_id, task["case_id"], now)
        elif attempt["status"] not in {"failed", "interrupted"}:
            return "ignored"
...
def _interrupt_attempt_in_transaction(
    ...
    updated = connection.execute(
        ...
        f"finished_at = ?, revision = revision + 1 WHERE attempt_id = ? AND status IN {statuses}",
...
def _settle_task(connection: Any, task: Any, attempt_id: str, now: str, *, succeeded: bool) -> None:
    ...
    updated = connection.execute(
        ...
        "WHERE task_id=? AND revision=? AND status IN ('running', 'cancelling')",
        (*values, task["task_id"], int(task["revision"])),
    )
    if updated.rowcount != 1:
