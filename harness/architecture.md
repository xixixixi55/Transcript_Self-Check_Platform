...
新增能力、同类输入适配、核心链路替换或技术方案设计开始前，MUST 先搜索项目内已稳定解决相同用户结果或核心处理目标的实现。搜索不能只看名称和目录，还要比较输入输出、调用阶段、错误模型，以及安全、性能、缓存、一致性和生命周期等非功能约束。

- 默认复用稳定先例的处理顺序、统一模型、失败语义和非功能约束，把格式差异限制在适配边界；复用不等于复制代码或继承已知缺陷。稳定先例应有现行规格、自动化回归、生产使用或明确验收证据。
- 只有安全、正确性、规模、平台或生命周期约束存在实质差异时才可偏离，并在 tasks 或设计工件中记录参考先例、不适用证据和验证方式。
...
- ≤400 行是源码文件的推荐目标，不是强制拆分线。
- 400–600 行的高内聚模块允许保留，无需为了 LOC 拆分。
- 600–800 行必须评估是否存在自然职责边界；决定保留时，必须在 `scripts/lint-arch.ts` 的 `FILE_SIZE_JUSTIFICATIONS` 中登记有界说明。
...
- 分层编号只表示允许的依赖方向，不表示每项功能必须机械创建 Repo→Service→Controller→Route 四层文件。
- 允许业务在不违反依赖方向和公共契约的前提下省略没有独立职责的中间层；禁止为形式上的分层创建 pass-through 模块。
