...
- **AND** 当前结构化事项完成并推进，或用户从“已填内容与待办”切换到另一个结构化事项后，本次页面会话均提供步骤导航：存在回复框时，“返回上一步”位于框内左侧，确认并进入下一步或返回最新当前步骤位于框内右侧，并分别使用向左、向右的方向图标；没有回复框时仍在助手提示之后保留适用入口。用户可重新查看或修改来源步骤的既有结构化控件，来源事项尚未完成时仍可返回，图片上传等高内容不得把入口推到回复区之后；步骤导航不得回滚已保存草稿、把已完成事项重新计入待处理数量或持久化为新的业务事实
...
- **AND** 用户可以从“已填内容与待办”进入任一当前待处理事项、返回本次会话已完成的既有结构化控件，或进入完整审核编辑修改其他既有内容，不被强制锁定在固定的一题一页顺序中
