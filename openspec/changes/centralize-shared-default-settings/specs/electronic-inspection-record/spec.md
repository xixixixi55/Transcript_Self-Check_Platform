...
- **WHEN** 用户在案件审核编辑页修改文号、委托单位前缀、检查地点、检查方法、检查硬件设备、检查人员、附件1提取方式或光盘编号并保存草稿
- **THEN** 修改只进入当前案件草稿
...
- **AND** 已有案件不因默认设置更新而回写
- **AND** 非空附件1提取方式固化为新案件快照，附件1预览、归档投影和正式文书优先使用该快照
- **AND** 附件1提取方式默认值为空时，继续按案件检查硬件设备和文件哈希算法生成既有描述
...
系统 MUST 从最终 `ArchiveManifest` 生成附件1数据行，并使用新案创建时固化的附件1提取方式快照；案件没有非空快照时继续使用既有硬件与哈希算法描述。

#### Scenario: 附件1使用案件提取方式快照

- **WHEN** 新案件创建时存在非空附件1提取方式默认值，随后完成归档或打开附件1预览
- **THEN** 系统将该值固化到案件，并在附件1各行和正式文书中使用同一提取方式
- **AND** 后续修改笔录默认设置不得回写该案件
- **AND** 案件快照为空或存量案件缺失该字段时，系统继续按 `inspection.hardware_device` 和案件文件哈希算法生成既有描述
