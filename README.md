...
文枢使用内嵌 SQLite 和本地文件系统共同保存数据，无需安装或运行独立数据库服务。

| 存储位置 | 保存内容 |
|---|---|
| SQLite | 案件与草稿、来源登记、任务进度、编辑租约、归档计划与尝试、发布证据、模板登记及清理记录等业务状态 |
| 本地文件系统 | 用户选择的原始报告和图片，以及生成的 Word、RAR、日志、缓存和受控临时文件等大文件 |

SQLite 不保存完整原始报告、Base64 图片、RAR 或 Word 文件，只保存必要的结构化业务数据、状态、文件引用和校验证据。Windows 正式环境的工作台数据库通常位于：

```text
%LOCALAPPDATA%\文枢\data\instances\<部署标识>\workbench.sqlite3
```

该数据库包含案件业务数据和本机路径引用，应按敏感业务数据保护。备份或迁移前应先退出文枢，并保存同一时点的应用数据目录；位于外部目录的原始报告、图片和已导出 Word/RAR 仍需按其实际位置另行备份。不要单独删除、替换数据库，或将其与不配套的历史工作区文件混用。
...
| 报告解析 | BeautifulSoup4 |
| 归档压缩 | WinRAR CLI |
| 存储 | 内嵌 SQLite（业务状态）+ 本地文件系统（报告、图片及生成文件） |
