...
- **AND** 材料页“设备类型”或“检材类型”优先于“数据类型”；仅有 `Android设备` 数据类型时保持待确认，不再映射为手机；缺少可靠类型且 IMEI1/IMEI2 为两个有效、不同的 15 位数字时可兜底推断为手机并保留推断诊断
...
#### Scenario: 平航 iOS 使用面向用户的手机型号

- **WHEN** 平航材料页明确为 iOS，并同时提供“手机品牌”“手机内部型号”和“手机型号”
- **THEN** 系统将“手机型号”直接作为设备显示名和型号，不再拼接手机品牌，也不优先使用内部型号
- **AND** iOS 缺少“手机型号”时回退既有品牌与内部型号规则；Android 始终保持“手机品牌 + 手机内部型号”的既有映射
...
- THEN `AttachmentPlan` 按 Manifest 中每个实际 part 生成一行数据：
  ...
  - 电子数据 = 实际 part 文件名
  - 来源 = 审核后的 `evidence_number` 去重并保持当前顺序；一至十个编号使用“、”拼接，超过十个编号时按人工填写原文使用“第一个检材编号至最后一个检材编号”，不得解析、补齐或重新生成编号，最后追加“检材内提取”；同一来源文本供各 part 行使用，不声称每个 part 独立对应一个检材编号
...
#### Scenario: 附件1来源按检材数量换行展示
- WHEN 审核后的有序检材编号数量不超过十个
- THEN Word 来源框逐个换行展示检材编号，除最后一个编号外保留顿号，并在编号后单独一行展示“检材内提取”
- WHEN 审核后的有序检材编号数量超过十个
- THEN Word 来源框第一行展示人工填写的“第一个检材编号至最后一个检材编号”，第二行展示“检材内提取”
- AND 解析预览、归档投影与正式 Word 使用同一来源文本格式规则

#### Scenario: 附件1按三条分卷上限优先填满前页并承载签名区
- WHEN 最终 `ArchiveManifest` 包含一条或多条实际分卷
- THEN 附件1按顺序每页最多放置三条分卷数据并优先填满前页；最后一个数据页少于三卷时，在最后一卷的下一行于“电子数据”列显示“以下空白”，再用普通空白行补足三个分卷槽位，所有补位行均不得使用斜线；末页恰好三卷时第三卷后直接进入签名区，不生成第四个“以下空白”行
- AND 当前系统最多生成五条分卷；三条分卷生成单个“3 条分卷 + 签名区”页面，四条和五条分卷分别按 `[3,1]`、`[3,2]` 分页，附件1最多两页
- AND 不得生成不含分卷数据、只含固定手写签名区的独立页面
- AND 固定手写签名区只出现在最后一个数据页，位于补位区之后并贴近页面底部，不得随数据量减少而向上浮动
- AND SHA-256 的64位摘要在附件1中保持与 MD5 相同的16磅字号，移除数据单元格的模板遗留空段落，并仅通过无重叠的字符宽度调整保持相同分页规则和完整摘要显示
...
- **THEN** 未导出案件不展示或调用删除条件预检，直接弹出“确认删除该案件？”并使用“确认删除”按钮，正文以“删除后”说明不可恢复范围；统一导出成功后的归档入口保留“确认归档该案件？”与“确认归档”
- **AND** 仅统一导出成功后直接显示“归档案件”推荐操作；其他状态的更多菜单只显示一个“删除案件”入口
...
- **AND** 当前事项在本次页面会话中完成后，当前对话区不累积用户完成回执或修改按钮，只展示当前推荐事项、结构化回复控件和必要的事项切换说明；除按案件隔离的未提交快捷批量检材输入草稿外，不得保存逐字输入或新增持久化聊天记录
- **AND** 当前案件中明确由用户输入、选择、修改、确认或上传形成的已完成结构化事项统一进入“已填内容与待办”面板，并展示当前用户最终值、保留返回对应既有控件的修改入口；报告识别、系统默认或系统计算且未经用户办理的内容不得出现在“已填内容”中，但仍按既有规则显示在 Word 内容预览；步骤索引不进入案件草稿或服务端，刷新或重新进入时仍可从持久化步骤检查点和当前草稿事实恢复可回访的用户事项
...
- **AND** 文本型当前事项继续把每次输入实时写入现有报告草稿和自动保存，但输入首个字符或后续 `change` 不得自动切换事项；用户可以按下非输入法组合态的 Enter，或点击输入区内提供明确 Tooltip 与可访问名称的圆形确认按钮来确认并推进，两种方式必须触发同一完成行为；普通多行文本的 Shift+Enter 继续用于换行，快捷批量检材输入框的普通 Enter 只插入换行且不得推进事项，只读或导出忙碌时确认按钮必须禁用
...
- **AND** 刷新或重新进入案件时，预览根据当前报告草稿重建；除可重建的结构化步骤参考与游标，以及独立于 Word 预览和案件事实的未提交快捷批量检材输入草稿外，不还原逐字对话、字段值副本或已失效的过程状态
...
- **THEN** 獬豸助手明确提示每个非空行只表示一项检材，普通 Enter 只在输入框中插入换行，不得确认当前事项或推进步骤；提交继续由现有显式解析与确认按钮触发；格式为“设备名称手机一部（无法提取原因）检材编号”或“设备名称平板一部（无法提取原因）检材编号”，原因必须使用中文全角括号且检材编号位于行末
...
- **AND** 任一非空行格式错误、批次内检材编号重复或与现有检材编号重复时，页面按行说明问题并阻止整批写入，保留用户原始输入及本地草稿
- **AND** 用户确认预览后，系统把整批检材追加到现有 `introduction.evidence_list`，不得覆盖现有检材，并继续调用同一报告更新、自动保存、字段来源和完整性确认链路，同时清除本案件的批量输入草稿
