...
- [x] 调整当时的 `ReviewActionBar.tsx`（后续已移除）与 `packages/frontend/src/reviewWorkspace.css`，使用返回、保存和 Word 文件图标构成紧凑的圆形操作按钮。
- [x] 更新当时的 `reviewWorkspaceComponents.test.tsx`（后续已拆分），覆盖纯图标呈现、无障碍名称及既有忙碌状态行为。
...
- [x] 调整 `packages/frontend/src/pages/CaseRecordGeneratePage.tsx`、当时的 `ReviewPendingSummary.tsx`（后续已移除）与 `packages/frontend/src/reviewWorkspace.css`，将有待核对项的摘要移入审核表单右侧随屏栏；空间不足时提供可展开的右侧悬浮入口，并为章节定位预留不被遮挡的滚动偏移。
...
- [x] 更新当时的 `reviewWorkspaceComponents.test.tsx`（后续已拆分）、`RecordEditorForm.test.tsx` 及必要的页面测试，覆盖有/无待核对项的右侧栏/悬浮入口、定位回调、标题内红色提醒、尾部空白警告显示条件及委托单位双列结构。
...
- [x] 运行 layout 机械扫描、定向 Vitest、架构检查、TypeScript 类型检查、`verify:quick`、scoped strict docs 与 `git diff --check`，并在真实浏览器中联合检查桌面与窄屏布局。
...
## 第二十三阶段：案件工作台冷调蓝色视觉统一

### 目标与验收标准

- [x] 案件工作台正文采用明亮中性画布、白色表面和冷调靛蓝操作锚点，形成清新、舒适且克制的层次感；Sidebar 与首页原有紫色风格保持不变。
- [x] 案件卡、上传报告目录入口、标题区控制项和分页具有可区分的表面层级；不使用满屏渐变、装饰图形或夸张动效。
- [x] 保留案件列表、状态、来源目录校验、刷新、上传解析、分页和案件操作的现有语义与调用链。
- [x] 桌面与窄屏均保持标题、控制项、案件卡和上传入口清晰可用，无新增横向溢出。

### Layer 11–12：工作台表面与响应式样式

- [x] 调整 `CaseWorkbenchPage.tsx`、`caseWorkbench.css` 与 `platformShell.css`，以工作台局部 Token 建立冷调靛蓝操作色、单层柔和阴影和统一圆角，不修改 Sidebar 与首页紫色 Token。
- [x] 案件卡与上传入口吸收参考图的大圆角白卡、长柔影、小图标锚点和少量暖色点睛；保留专业信息密度，不引入金融图表或大面积深色面板。
- [x] 根据最新参考图提高卡片纯白表面占比，移除普通案件与上传入口上的暖色光斑和无语义圆点，以冷调蓝操作锚点和紫灰柔影维持克制层次。
- [x] 根据反馈移除工作台普通操作的青绿色，统一替换为与参考图和 Sidebar 协调的冷调靛蓝；仅保留成功状态的语义绿色。
- [x] 运行工作台定向 Vitest、前端 TypeScript 类型检查、Impeccable 机械检测、真实浏览器桌面/窄屏联合验收与 scoped `git diff --check`。

### 影响范围

- 影响 Layer 11 FE_Components 与 Layer 12 FE_Pages/样式；不修改共享类型、路由、后端接口、数据模型、持久化、上传解析、案件状态机或 Word 导出合同。
- 本阶段只迁移案件工作台视觉，不修改审核编辑页、管理页或全局 Ant Design `colorPrimary`。

### 第二十三阶段验证记录

- 工作台、案件卡与目录选择器定向 Vitest 3 files / 31 passed；现有来源目录校验、刷新、上传解析、分页、案件操作和并发导出回归保持通过。
- 前端 TypeScript 类型检查通过；Impeccable 机械检测只报告本次修改前已存在的 Sidebar `margin-left` 布局过渡，本次工作台样式无新增告警。
- 真实浏览器使用合成测试案件联合检查 1280×720 与 620×900：工作台保持近白中性画布和冷调靛蓝操作锚点，Sidebar 选中态仍为原紫色；案件卡与上传入口统一为 20px 大圆角纯白卡、长柔影和小图标锚点，桌面三列与窄屏单列均无页面或内容横向溢出，窄屏标题区自动改为纵向布局。
- 最新参考图反馈收敛后，案件卡与上传入口均为纯白背景并移除暖色光斑及无语义圆点；真实浏览器复查 1280×720 与 390×844 均无横向溢出，侧边栏原紫色选中态保持不变。
- 青绿色反馈收敛后，工作台普通交互统一为 `#4a62d0` 冷调靛蓝，按钮白字对比度 5.32:1、浅色状态标签对比度 4.73:1；成功状态继续使用语义绿色。
- `verify:quick` 通过，含架构、类型、治理测试、文档和仓库资产检查；scoped strict docs 14 checks / 0 drift。
- scoped `git diff --check` 通过；本阶段未修改全局 `colorPrimary`、业务请求、共享合同或后端实现。

## 第二十四阶段：完整审核工作区层级收敛

### 目标与验收标准

- [x] 压缩完整审核页头高度，将引导模式切换与结构摘要入口收入页头，不再使用独立悬空工具行。
- [x] 将压缩时机和介质编号整合为单一“归档准备”面板，降低普通信息块的视觉权重，保留原有压缩、编号和导出行为。
- [x] 页头、归档准备与审核表单使用同一内容宽度；收紧章节卡片和底部操作栏，修正文号前缀、序号与后缀的松散排布。
- [x] 运行完整审核页定向 Vitest、前端 TypeScript 类型检查、Impeccable 检测与 scoped `git diff --check`。

### 影响范围

- 影响 Layer 11 FE_Components、Layer 12 FE_Pages 与审核页局部样式；不修改共享类型、路由、后端接口、数据模型、持久化、归档状态机或 Word 导出合同。

### 第二十四阶段验证记录

- 完整审核页、编辑表单和归档完成面板定向 Vitest 3 files / 45 passed；显式回归引导模式往返、介质编号草稿保留、压缩决策、归档映射和 Word 导出行为不变；修复可访问名称后失败页级文件 19/19 通过。
- 前端 TypeScript 类型检查通过；Impeccable 对变更 TSX 扫描返回 0 项；scoped `git diff --check` 通过。
- 真实浏览器使用 SYNTHETIC/TEST 案件复核：1440×900 下页头、归档准备和审核表单同为 1088px 且无水平溢出；390×844 下归档提示改为正文与操作分行布局，页面、工作区和准备面板的 `scrollWidth` 均不超过 `clientWidth`。

## 第二十五阶段：管理页视觉统一

### 目标与验收标准

- [x] 取证硬件设备、检查人员、笔录模版和笔录默认设置四个管理页复用案件工作台的近白画布、冷调靛蓝操作锚点、标题层级、20px 白色表面和单层柔影。
- [x] 表格工具区、搜索、新增操作、表单分区与底部操作区形成一致层级，保留现有加载、空态、错误、编辑、删除、默认模版和保存语义。
- [x] 桌面与窄屏下标题、工具区、表格和默认设置表单无新增横向溢出，Sidebar 与首页原紫色视觉保持不变。

### 实现与验证

- [x] 调整四个管理页及 `DeviceManager`、`InspectorManager`、`platformShell.css`，仅统一页面骨架和表面样式，不修改请求、路由、数据或保存逻辑。
- [x] 根据反馈移除设备列表、人员列表和已校验模版标题下的辅助说明，并收紧标题区间距。
- [x] 根据反馈将笔录默认设置的重新加载与保存操作改为带 Tooltip 和无障碍名称的圆形图标按钮，并复用审核编辑操作栏的全局滚动跟随方式。
- [x] 运行管理组件与平台外壳定向 Vitest、前端 TypeScript 类型检查、Impeccable 检测、桌面/窄屏浏览器验收及 scoped `git diff --check`。

### 影响范围

- 影响 Layer 11 FE_Components、Layer 12 FE_Pages 与平台局部样式；不修改共享类型、后端接口、数据模型、持久化、模板合同或 Word 导出。
- 本阶段延续第二十三阶段已确认的案件工作台视觉语言；全局 `colorPrimary`、Sidebar 和首页视觉 Token 保持不变。

### 第二十五阶段验证记录

- 管理组件与平台外壳定向 Vitest 5 files / 38 passed；设备和人员增删改、模板默认/删除/微调/重命名、默认设置保存/冲突及导航高亮回归保持通过。测试输出仅保留既有 Ant Design/jsdom 弃用与布局测量警告。
- 前端 TypeScript 类型检查和 `lint:arch` 通过；Impeccable 检测只报告本次修改前已存在的 Sidebar `margin-left` 布局过渡，管理页新增实现无机械告警；scoped `git diff --check` 通过。
- 真实浏览器使用现有 SYNTHETIC/TEST 数据联合检查 1440×900 与 390×844：四页均使用 30px/26px 主标题、近白画布、20px 白色表面和冷调靛蓝操作锚点；页面与主内容无横向溢出。设备/人员表格在窄屏改为卡片内部横向滚动，避免字段逐字竖排；默认设置两段表单保持单列可用。
- 浏览器控制台仅观察到本次范围外既有 Ant Design Menu `children`、`findDOMNode` 与 React Router future flag 警告；未发现管理页新增运行时错误。
- 辅助说明移除反馈后，设备、人员和模版管理定向 Vitest 3 files / 12 passed；浏览器复核 1440×900 与 390×844 下三处说明均不存在、标题下边距归零且无页面级横向溢出；scoped `git diff --check` 通过。
