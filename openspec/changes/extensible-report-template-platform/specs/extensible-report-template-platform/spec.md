...
阶段一 MUST 只允许最终检材类型 `phone`/`tablet`。人工确认始终优先；报告“手机品牌”规范化后精确为 `iPhone`/`iPad` 时分别自动确认 `phone`/`tablet`，并优先于不可靠的“设备类型=手机”。明确平板或同时命中手机/平板的类型冲突继续受保护；其后只有 IMEI1 与 IMEI2 均为有效、不同的 15 位数字才可推断 `phone`，以报告确认状态保留推断诊断并直接通过检材类型导出门控；报告“设备类型=手机”仅作为最后一个自动候选。单个、非法或重复 IMEI 不得触发双 IMEI 推断，其余情况由审核页面确认。主取证软件无法可靠识别时允许审核页面分别编辑名称和版本，但确认前不得正式导出。WinRAR 缺失或不可调用时允许上传、解析和编辑，禁止自动压缩、禁止最终正式导出、不生成 `ArchiveManifest`，且不降级 ZIP。0 张图片不生成附件二页面，正偶数图片按页面计划生成，奇数图片阻止导出；附件二缺失不重排附件三编号。人员快照按用户选择顺序一人一行；附件一每页最多三条分卷并从前向后优先填满，最后一个数据页少于三卷时在最后一卷后生成“以下空白”和所需普通空白补位行以补足三个分卷槽位，恰好三卷时不生成第四个补位行，再于页面底部承载不可拆分的人员框，不得生成签名独立页。附件一来源在一至十个检材时逐项换行，超过十个检材时按人工填写原文折叠为“第一个检材编号至最后一个检材编号”，并将“检材内提取”置于下一行。SHA-256 的64位摘要保持16磅字号、移除模板空段落并以无重叠字符宽度适配，分页规则与 MD5 一致。`shadow` 不调用 WinRAR、不执行真实重复压缩、不产生第二份正式文书。
...
#### Scenario: 十一个及以上检材的附件一来源折叠展示

- **WHEN** 审核后的有序检材编号超过十个
- **THEN** 附件一来源只使用人工填写的第一个和最后一个检材编号原文展示“第一个检材编号至最后一个检材编号”
- **AND** “检材内提取”在范围文本后的下一行展示，编号不得被解析、补齐或重新生成
