...
## AD-008：委托单位前缀独立保存并在文书边界组合

`introduction.entrust_unit` 继续只保存报告识别出的委托单位，新增可选的 `introduction.entrust_unit_prefix` 保存当前案件使用的前缀。前缀同时进入部署实例共享默认值，新建案件初始化时复制到草稿；用户编辑前缀时随草稿成功保存进行稀疏共享默认值更新。与其他共享字段不同，该字段允许空值更新，以支持显式清除前缀。
