...
解析层可以保留未来标识类型，但阶段一的 `MaterialDisplayPolicy` 只输出手机的合法 IMEI1/IMEI2 或平板的合法序列号。分类必须优先使用报告明确且可靠的类型；多个明确类型冲突时保持 `pending_confirmation`，不得按读取顺序取首值。无可靠类型时，只有 IMEI1 与 IMEI2 均为有效、不同的 15 位数字才兜底推断手机并记录 `MATERIAL_TYPE_INFERRED_FROM_DUAL_IMEI`；该推断使用 `confirmed_by_report`，可直接通过检材类型导出门控，同时允许用户修改。单个、非法或重复 IMEI 不推断，明确平板或类型冲突不被覆盖。最终导出前每个材料必须是 `phone` 或 `tablet`；原始 identifiers 始终保留，不在渲染层补字段或删除数据。
...
页面定位采用类似现有 legacy/new 的 v1 快速路径：报告信息页使用 v1 固定锚点 `0_1.json`，但必须同时校验赋值变量、`Info.type` 和报告字段标签，不能只相信编号；案件页由导航中严格 Base64 解码为“案件信息”的唯一 Table 节点定位；设备页由导航中全部 `viewType = DeviceInfo` 节点及其 `dataIndex`/`rangeCount` 定位；持有人页只从与单一 DeviceInfo 共享顶层设备作用域的唯一“机主信息” Table 定位。除这些核心页外不枚举 ViewData：

| 规范化目标 | 平航页面语义 | 规则 |
|---|---|---|
| 案件名称、编号、摘要、送检人员、送检单位、创建时间 | 导航显示标签为“案件信息”的唯一 Table | 导航唯一定位后再校验案件名称/编号等标签；“送检人员”“送检单位”分别投影现有委托人员和委托单位；重复导航节点产生 issue |
| 报告完成时间、主软件名称与版本 | v1 `0_1.json` Table + confirmed adapter rule | 同时校验固定赋值和数据取证软件版本/报告导出软件版本/报告完成日期标签；版本取“数据取证软件版本”，名称按用户确认的 v1 适配规则默认为“平航手机多路分析取证软件”，确认来源标记为 `confirmed_by_user` |
| 检材名称、编号、型号、标识符、类型、取证时间 | `Info.type = DeviceInfo` | 每个页面形成一个材料候选，保留 IMEI/IMEI2/序列码来源；“设备类型”/“检材类型”优先于“数据类型”，仅有 `Android设备` 数据类型不归类为手机；缺少可靠类型时适用统一双 IMEI 兜底；全部材料时间有效时，以最早“取证开始时间”和最晚“取证结束时间”形成检查起止时间 |
| 检材持有人 | 与该 DeviceInfo 共享顶层设备作用域且显示名为“机主信息”的唯一 Table | 只读取明确“用户姓名”并写入该检材 `holder_name`；电话号码、证件和其他人员字段不参与；作用域内无法唯一配对时安全失败 |
| 附件二检材照片 | 无自动映射 | 继续使用现有人工上传和显式材料绑定 |

多个 `DeviceInfo` 页面按已验证的导航顺序生成 `Material[]`；导航缺失、页面重复、检材编号重复或顺序无法确定时保留候选并阻止正式导出，不能按文件枚举偶然顺序猜测。平航 v1 优先读取明确“设备类型”/“检材类型”；仅有“数据类型=Android设备”时保留原值但不产生手机确认。无可靠类型且存在两个有效、不同的 IMEI 时由统一策略推断手机并产生带诊断的 `confirmed_by_report`；仅一个 IMEI 时仍待确认，明确平板或冲突类型不得被覆盖。平航检查起止时间按用户确认的报告语义取全部材料中最早的“取证开始时间”和最晚的“取证结束时间”；只有每个材料的起止时间均可解析且不倒置时才自动投影，否则留空待审核，不使用只有日期的报告完成日期补造分钟。案件创建时间和报告完成时间仍作为独立来源事实保留。报告未明确绑定取证硬件时，平航解析初值保持为空并由现有审核/共享默认设置补充，不得套用美亚硬件默认值。报告本身未提供主软件名称时，已确认的 v1 适配规则使用“平航手机多路分析取证软件”作为用户确认默认值，并继续把报告中的“数据取证软件版本”作为版本事实；该规则不得扩展到未命中 v1 的平航变体或其他厂商报告。
...
首次登记、来源替换、原生选择和旧版解析统一执行本机路径安全、输出隔离及报告结构检查。来源目录白名单、配置加载、来源精确令牌和浏览器开关偏好已移除，旧偏好不再影响导入。共享请求定义在 `packages/shared/types/sourceRequests.ts`，请求构造器在 `useSourceRequests.ts`。

原生选择器仍使用受控 Windows 桌面桥接，选择后在后端立即登记来源并启动解析；取消无副作用，绝对路径不进入浏览器 DTO。导出目录的一次性令牌独立保留，包括过期、路径绑定和重复使用检查。
...
`current-template-v1` 使用 `ooxml-package-sha256-v1`。算法先安全读取有效 DOCX ZIP，拒绝加密、重复或大小写折叠冲突条目、绝对路径、反斜杠路径和 `..` 路径；然后按完整条目名称排序，将条目名称长度、名称、解压内容长度和解压后的原始内容与固定域标记一起计算 SHA-256。它忽略 ZIP 压缩参数、条目顺序和容器时间戳，但不忽略任何实际包部件，也不做 XML 语义归一化。原始 DOCX SHA-256 仅用于诊断，不参与 Profile 匹配。模板资产的任何语义修改都发布新版本：`1.0.0`、清理后的 `1.0.1` 和正文边界初步平衡的 `1.0.2` 历史资产保持只读且仅供既有案件重导出，不能用于新选择、默认设置或删除；主标题、结构标题层级和粗横线通过 Word 原生渲染复核的 `1.0.3` 成为当前内置默认。启动迁移幂等更新仍指向任一旧内置版本的共享默认值，不改写案件引用或用户选择的其他默认模板；即使新版注册后发生中断，下一次启动仍可完成默认迁移。

仅发生无语义 ZIP 重打包的模板具有相同包指纹；甲方认可参考文件作为视觉基准但因实际 OOXML 部件不同而不匹配 `current-template-v1`。后续对主标题、结构标题和 VML 横线锚点的语义修改产生新的包指纹并作为 `1.0.3` 提交，原 `1.0.2` 字节另存为不可变历史资产。

Renderer 先写入临时输出副本，再原子替换正式输出。输出副本完整删除评论部件、评论标记、评论关系和对应内容类型覆盖，删除未使用的自定义属性及关系，删除不依赖的 `docVars`，并将核心属性净化为通用系统名、空标题和重置 revision；不产生修订。模板中用于固定手写行布局的通用隐藏文字（一个“份”和四个空格）保留，不含业务数据。`1.0.1` 源模板在资产层删除全部评论结构与附件二示例图片，但保留原空白宿主段落作为受控图片区；`1.0.2` 将带负右缩进的直接正文段落改为左右边界成对平衡，并把附件一固定表格设为相对页面居中。`1.0.3` 进一步移除主标题用于伪居中的前后制表符并采用真正的段落居中，按固定文字锚点恢复一级/二级结构标题的相对层级，同时依据 A4 页宽、左右页边距和既有 VML 线段长度重新计算首页及页脚横线的等距端点。该确定性变换不改变段落可用宽度、表格列宽、分页锚点、VML 文本框、页眉页脚内容或线型；Profile 同时校验这些可见版式语义。
...
用户从模板管理页选择一个已校验且非历史只读的源版本，前端只提供模板显示名称、文书固定标题、正文默认字体和字号的白名单控件与近似视觉预览。后端从源 DOCX 副本施加确定性 OOXML 修改，生成新 ID/版本资产；源资产、源登记和案件引用不变。新资产重新计算包指纹，并执行与上传模板相同的 `current-template-v1` 完整结构校验后才记为已校验。前端预览必须标记为非 Word 最终分页预览。

这一决策允许解决高频文案和字体微调，同时不开放附件表格、VML、图片区、分页、重复块或任意字段绑定。备选方案是引入浏览器富文本/Word 式设计器；不采用，因为浏览器排版无法稳定复现 Microsoft Word 的 VML、表格和分页行为，且会越过当前固定 Profile 安全边界。
...
`Attachment1PagePlan` 只规划包含 `archive_rows` 的数据页，最后一个数据页同时承载补位区和固定手写行；不得规划零条分卷数据的独立 `inspector_final` 页面。附件一固定手写行复制甲方认可模板的原始文字、单元格、合并、边框、字体、字号和留白，不写入 `InspectorSnapshot[]`。`signatureBlankRowCount` 由规划器按最后数据页的本页卷数决定，始终等于 `3 - len(lastPage.serialRows)`，因此末页为1、2、3卷时分别复制2、1、0条模板补位行；恰好三卷时第三卷后直接进入签名区，不再生成“以下空白”行。存在补位行时，Renderer 清除旧 VML 斜线，在第一条补位行的“电子数据”列写入“以下空白”，其余补位行保持空白。首页三卷末页把被取消的模板补位行高度并入签名行，使签名区继续贴近页底；续页末页以 4500 twips 为补位区基准总高度，并按超出三行的来源文本每行收缩 180 twips（单条补位行不低于 600 twips），再按补位行数均分。这使一卷尾页与二卷尾页在来源内容增高时仍为固定手写签名区让出版面，并将签名区稳定保持在页面底部。所有附件一数据单元格在写值后移除模板遗留的额外空段落；SHA-256 哈希仍使用与 MD5 相同的 16 磅字号，仅把字符宽度缩放为 54%，让完整 64 位摘要稳定落入三行且不重叠，从而保持与 MD5 相同的 `[3]`、`[3,1]`、`[3,2]` 页面计划。current-template-v1 的每个数据页最多承载三条 `archive_rows`，前页优先填满三条；四卷与五卷分别固定为 `[3,1]`、`[3,2]`，规划后的附件摘要页数直接取页面计划长度。正文“（八）检查人员”仍由有序快照动态生成。
...
阶段一只登记并使用固定的 `current-template-v1` 和固定 TemplateProfile；只允许当前 DOCX Renderer 的受控扩展。通用模板设计器、通用重复块 DSL、任意 DOCX 自动绑定、可视化模板编辑和无标记模板识别全部是阶段三接口预留，不在阶段一实现或静默启用。模板填充只在 Profile 允许的段落、表格、文本框和图片区内进行；保留 VML 宿主段落、关系、普通分页符和表格边框。对正式模板的批注/示例图片卫生清理采用确定性 OOXML 脚本和不可变版本升级，不修改甲方参考 DOCX，也不把清理后的字节冒充旧模板版本。
...
- 检材阶段一只允许 `phone`/`tablet`；可靠且无冲突的明确类型可预选；缺少可靠类型时只允许两个有效、不同的 15 位 IMEI 兜底推断手机，其余在审核页确认。
