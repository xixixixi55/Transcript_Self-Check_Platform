...
## D2. 单次 inventory 与用户确认边界（后续演进覆盖）

**最终决策**：已归档的 `metadata-fingerprint-archive-path` 覆盖了本包早期的双扫描设计。同一 Worker 只构建一次完整 inventory，供规划、Manifest 输入统计与 WinRAR 共用；不得在 WinRAR 前后重复递归扫描。用户确认承担压缩期间不再修改来源的交互边界；WinRAR 或输出完整性校验实际观察到错误时仍安全失败。

**理由**：避免大目录被重复枚举，同时保留直接源性能目标和输出侧完整性门控。评审必须以 living spec 的最终合同为准，不能恢复已被后续变更移除的前后全目录扫描。
...
## D9. 工作台直出与全局 Manifest 索引解耦

**决策**：`ArchiveManifestRepository` 在注入工作台数据库时，只从 `archive_publish_intents` 投影可用 Manifest 记录，不再读取、创建、锁定或重写 `output/compressed/.archive-manifest-index.json`。直接发布的 RAR 继续位于用户所选报告目录的上一级，位置绑定和恢复状态继续写入 SQLite。未注入数据库的旧调用保留原文件索引、跨进程锁和失败关闭行为；旧内部目录发布仍显式使用文件系统锁。

**理由**：直出归档已经把物理成果放到用户选择的目录旁，当前案件库也已经持久化发布意图、文件集摘要和实际位置。继续把全局 `output` 索引放进成功链路会形成第二套部署范围的状态边界，并让不属于当前案件库的历史文件错误阻塞新任务。

**兼容与迁移**：本轮不删除、不搬迁现有 `output` 内容，也不把旧索引导入当前案件库。数据库证据不足时仍按现有发布证据错误安全失败；只有无数据库兼容流程继续依赖旧索引。这样无需破坏性迁移，旧资产可由维护流程另行审计和清理。
