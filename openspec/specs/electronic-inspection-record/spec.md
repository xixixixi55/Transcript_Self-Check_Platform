...
#### Scenario: 历史预览与当前对话使用可交换分栏

- **WHEN** 对话式引导视图存在已经完成、自动处理、正在后台处理或当前需要用户操作的事项
- **THEN** 桌面审核工作区铺满所在主工作区的可用宽度并使用等宽左右分栏，左侧展示完整历史预览，右侧展示当前对话，历史区和对话区分别滚动且当前操作与全局操作持续可达
- **AND** 页面提供明确的左右位置交换操作；交换后历史预览与当前对话同步改变视觉顺序、DOM 顺序和键盘顺序，但不得卸载当前回复、清空本次会话展示状态或修改任何业务数据
- **AND** 当前对话区采用紧凑 Chat 形式展示獬豸助手身份、可替换的角色占位、当前推荐事项、结构化控件和主要操作；没有用户事项时显示等待后台处理、办理完成或现有恢复入口
- **AND** 历史预览按办理顺序完整渲染已经完成的用户操作、报告自动识别与系统自动沿用的信息摘要、后台任务阶段变化和已经恢复的异常，不重复展开完整审核表单，也不改造成逐句聊天记录或消息气泡
- **AND** 历史预览中的可编辑业务信息 MUST 从当前报告草稿投影最终生效值；报告识别或系统带入且未修改的字段显示现值，用户填写或修改后的字段显示用户修改后的现值，不并列保留已经被覆盖的旧识别值
- **AND** 当现有 `FieldState` 或检材来源明确表明最终值由用户填写、修改或补充时，历史预览 MUST 在对应字段或检材旁显示可见且可访问的“用户填写”标记；标记不得仅依赖颜色，也不得把报告识别或系统带入内容误标为用户输入
...
- **AND** 保存、来源、归档、导出和异常恢复等过程事实继续使用用户可理解的状态语句，不得机械改写为“字段名：值”；历史预览仍只是现有业务事实的展示投影
...
- **AND** 新增历史记录不得改变对话区滚动位置；用户在历史预览中阅读较早记录时不得被强制拉回最新记录
