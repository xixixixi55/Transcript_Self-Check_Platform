...
- [x] T031 删除页面层重复覆盖的归档状态、进度导航和生命周期展示测试，保留对应组件合同与页面接线回归。文件：`packages/frontend/src/pages/CaseRecordGeneratePage.test.tsx`、`packages/frontend/src/components/ArchiveDecisionPanel.test.tsx`、`packages/frontend/src/components/ArchiveStatusCard.test.tsx`，以及当时的审核工作区组件测试（后续已拆分）；验证：相关前端测试。
...
- [x] T035 删除工作台页面层重复验证的取消删除和重复导出 loading 场景，保留删除成功接线、导出成功接线、CaseCard 禁用/loading 合同与 Hook 请求合同；同时迁移 AntD Modal 的废弃属性以清除重复日志噪音。文件：`packages/frontend/src/pages/CaseWorkbenchPage.test.tsx`、`packages/frontend/src/components/CaseCardCompletion.test.tsx`、`packages/frontend/src/hooks/useArchiveCompletion.test.tsx`、当时的 `ReviewPreviewDrawer.tsx`（后续已移除）、`packages/frontend/src/components/WordDownloadNameDialog.tsx`；验证：相关前端测试与 typecheck。
