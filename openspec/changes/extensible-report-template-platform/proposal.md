...
依据 2026-09-08 反馈，移除来源目录白名单校验、浏览器开关偏好和来源授权请求参数。首次登记、原生选择、来源替换及旧版解析统一使用有效本机报告目录，保留基础路径安全、报告结构、输入输出隔离与来源上下文检查。导出目标目录的一次性授权独立保留。
...
- 增加来源目录授权模式开关；首页开关使用浏览器持久化偏好控制“任意本机目录”与“配置根目录校验”两种模式，安全路径、报告结构和输出区域校验始终保留。
