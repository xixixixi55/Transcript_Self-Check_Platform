...
- 最终结果：`PASS`。
- 审查范围：本提案、设计、任务、4 组差异规格；当前 v10 模式、来源/快照/发布/Word 事实；现行规格；以及相关活跃变更的直接合同冲突。
- 关键关闭项：C-01、C-02、C-03 及 H-01–H-05 全部关闭；无 Critical、High 或阻断性 Medium。
- 验证证据：变更严格检查、现行规格严格检查、`verify:docs:strict`、`git diff --check` 和 OpenSpec 状态全部通过。
- 时间合同：durable 时间、比较、CAS、审计和 retention 计算使用带时区 UTC；公共 API 返回带时区 ISO 8601；用户界面和人工验收按 `Asia/Shanghai` 展示；默认 30 天按连续 720 小时计算。
- 结论：允许提交第五阶段立项包；规划审查到此终止，不再增加新的规格冻结审查。PASS 只批准规划基线，不表示第五阶段产品实现已完成；后续必须从任务中的第 5A 阶段开始，并先执行活跃变更/模式重叠门控和当前 v10 事实复核，不得绕过任务批量实现。

## 第 5A 阶段实现前门控记录

- 门控名称：`第 5A 阶段实现前重叠门控和 v10 事实验证`。
- 审查基线：`3edcd2c41b732efbb8d264798e2c3d980be33e5a`；对应现有 tasks 的 `1.1 T024`，未新增任务编号。
- 门控结果：`PASS`。本次只完成活跃变更重叠审计、v10 事实核对和首批实现切片确认；没有修改产品代码、测试或模式，没有启动 Coordinator、enforce 或案件清理。

### 活跃变更重叠结论

| 活跃变更 | 实际交集 | v11/权威/策略冲突 | 顺序与门控结论 |
|---|---|---|---|
...
| Word/模板相关活跃变更（含 `template-2026`、`docx-vml-pagination`、`preview-export-correction`、`export-name-and-datetime-controls`） | 模板/渲染/文件名局部行为 | 未定义与 `word_artifact_id` 冲突的标识或新的权威；不改变模板公共合同 | 正式 Word 产物记录最终模板标识/版本；不阻断 |
| 其他当前活跃变更（Parser、UI、上传、Demo 和请求存活性变更） | 局部前端、解析或缓存文件 | 未引用工作台模式/迁移、v11、发布权威或保留策略 | 仅保留登记，不修改其他变更；潜在文件交集不阻断 |

结论：未发现其他活跃变更明确占用 `WORKBENCH_DATABASE_SCHEMA_VERSION = 11`、定义不可兼容迁移、替换 `archive_publish_intents` 权威、要求临时 `report_json` 作为唯一正式 Word 入口，或以不同持久行作为保留策略权威。已登记的合同交集不改变仅 Legacy、Canonical 未启用、Shadow 暂停边界。
...
- 三个时间均来自持久事实，缺失、冲突、未来时间或无法验证时安全失败；到期时间固定为 `expires_at_utc = retention_anchor_utc + (retention_days × 24 hours)`，不按无时区本地文本比较。
...
- 增加确定性清理预览/试运行：返回候选、跳过原因码、计划清理类别、明确保留类别、锚点、到期判断、任务/租约/恢复/冲突摘要和修订/摘要安全摘要。预览与 Coordinator 执行使用同一版本化资格规则，执行时在认领、文件删除前、SQLite 清理前和成功前重复校验。
...
本变更只新增差异规格，不提前修改现行规格；实现完成并通过归档门控后，才按仓库流程决定是否同步现行规格。
...
| Word/模板活跃变更 | Word 文件和模板版本事实 | 在 Word 产物持久事实完成前不得启用 `enforce`；不修改模板合同 |
| 其他活跃变更 | Parser、Legacy/UI、上传和预览局部交集 | 只登记依赖，不修改或归档；不得改变本变更的权威、Legacy/Canonical/Shadow 和 v11 门控 |

只存在潜在文件交集而没有公共合同冲突的活跃变更不自动阻断；上述直接合同交集必须在第 5A 阶段记录通过后才能进入实现。

## 验收边界

本变更只有在完整验证、清理/正式产物保护后端与 API 测试、工作台综合 E2E、合成数据人工验收、独立 Level 3 代码审查、最终审查和生产审查均形成证据，并确认归档就绪性后，才可进入 OpenSpec 归档。任何状态不明、数据库与文件不一致、所有权不明、发布/Word 权威缺失、稳定标识无法访问或 Canonical/Shadow 越界都必须阻断成功。
