...
export function buildInspectionEnvironmentStep(
  ...
  if (security.status === 'detected' || security.status === 'version_unknown') {
    ...
    return `启动${hardware}，${operatingSystem}启动正常，使用${name}（${version}）对${hardware}进行杀毒，未发现病毒，完毕后退出${name}。`
  }

  return `启动${hardware}，${operatingSystem}启动正常，安全软件待确认（版本号待确认），对${hardware}进行杀毒的结果待确认。`
}
