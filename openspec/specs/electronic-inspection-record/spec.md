...
内置模板以不可变版本保存。当前 `electronic-inspection-record@1.0.2` 不包含批注结构或附件二示例媒体，保留附件二空白定位段落、VML、分页、表格和动态图片渲染合同，并使用对称 A4 左右页边距、平衡的正文左右排版边界和相对页面居中的附件一固定表格。历史 `1.0.0`、`1.0.1` 资产继续供既有案件重导出，但不进入新案件可选列表，且不能再设为默认或删除。启动迁移将仍指向历史只读内置版本的默认模板迁移到 `1.0.2`，不改写已有案件引用或用户选择的其他默认模板。
...
- THEN 新案件默认引用 `electronic-inspection-record@1.0.2`
- AND 已明确引用 `1.0.0` 或 `1.0.1` 的案件继续使用对应历史资产，案件模板引用不变
...
- THEN 正文左右排版边界围绕页面中心平衡，附件一固定表格相对页面水平居中
