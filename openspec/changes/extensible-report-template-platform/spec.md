...
系统 MUST 先生成 `Attachment1Plan` 再渲染附件一。数据行数 MUST 等于最终 `ArchiveManifest` 的卷数；每个数据页最多四项，并从前向后优先填满四项，最后一个数据页在一至四项数据之后承载固定手写行；表头只在第一页出现且不得由 Word 自动重复；“附件1”只在第一页；来源框和提取方法框每个数据页分别生成合并框。总序号少于三条时，最后一个数据页可以按模板空白行补足视觉留白：一条分卷保留两条，二条分卷保留一条；总序号达到三条后不得保留斜线空白行。最后一页 MUST 同时包含至少一条分卷数据和固定手写行，不得生成只含固定手写行的独立页面。规划器 MUST 以每页最多四项、优先填满前页的确定性分页为固定手写行预留实际版式容量，不得为均衡末页而减少前页条数，也不得依赖 Word 自动分页后留下孤立签名页。固定手写行和其页面 MUST 设置不可拆页/保持完整，不写入动态检查人员。
...
#### Scenario: 四卷附件一不生成独立签名页

- **WHEN** 已验证的 `ArchiveManifest` 包含四条实际分卷
- **THEN** 页面计划只生成一页，该页依次包含四条分卷并在其后附带固定手写行
