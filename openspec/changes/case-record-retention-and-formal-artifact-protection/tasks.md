...
- [x] 3.1 **T022** 在 retention service 实现 anchor、`publication_verified_at` NULL-only CAS/v10 revalidation/enforce gate、Word `verified_at`、UTC aware/未来时间检查和完整 eligibility predicate；完成条件：unknown、publication verified time missing、revalidation 未完成/失败、Manifest 不一致、authority/ownership 缺失、活动任务/租约/恢复、未验证 Word/publication 均 fail-closed，`expires_at_utc` 使用 `anchor_utc + retention_days × 24 hours`，禁止使用无时区时间或普通 `updated_at`；验证：正反服务、revalidation、CAS、republish identity 和时区 fixture；证据：retention service tests。
  - 实现范围：新增 `CaseRetentionService` 及内部 publication authority helper；服务只读取 durable `case_retention_policies`，按 meaningful mutation、所有 verified publication 和 verified Word artifact 计算 `max` anchor 与连续 24 小时 expiry，并通过 deployment/case retention projection upsert 保存当前 policy/case revision。`CaseRetentionRepository.upsert` 已从 insert-only 修复为 deployment/case 唯一作用域的 durable projection upsert。
  - publication/Word 边界：历史 `publication_verified_at IS NULL` 只有在 controlled revalidator 同时证明 publication identity、file inventory、RAR/Manifest/MD5、fence、ownership、deployment 和 case 后，才调用既有 NULL-only CAS；失败保持 NULL，未创建 competing authority 或新 publication identity。Word verifier 要求 durable digest、size、Manifest digest 和 ownership 一致；内部结果不暴露路径。
  - eligibility 边界：缺失/异常/未来时间、publication/Word authority、Manifest/digest/file-set/fence/ownership、非终态、active task/lease/recovery/context/snapshot/cleanup run 均 fail-closed；只返回 durable policy mode 为 `enforce` 时的 `enforce_allowed` gate，不启动 Scheduler、preview 或清理执行。
  ...
  - 文档同步：living `openspec/specs/data-model.md` 已补充 3.1 retention service、受控 publication revalidation、UTC/future fail-closed、Word verification boundary 和 `enforce_allowed`；明确 preview 的公共 route/API/UI、Coordinator、物理文件复验和清理仍未交付。
  ...
  - 提交/推送：实现提交 `4f662ce`（`feat(retention): add eligibility and publication revalidation service`）及文档证据提交 `7503a3f`（`docs(openspec): record retention service push evidence`）均已推送 `origin/codex/demo-next-stage`。
- [x] 3.2 **T022** 实现确定性 preview/dry-run；完成条件：按 case ID 稳定排序，返回候选/跳过、原因、清理/保留类别、anchor/due、policy/case revision、任务/租约/恢复摘要和安全 digest，不含内部身份；验证：DTO snapshot 和敏感字段断言；证据：preview tests。
  - 实现范围：新增 backend-only `CaseRetentionPreviewService`，按当前 deployment 的 `case_shells` 以 `case_id ASC` 稳定扫描，并复用 `CaseRetentionService.evaluate_case()` 的 anchor、expiry、publication/Word authority、revision 和 blocker 资格判断；projection 返回 candidate/skipped/blocked、稳定 blocker、计划清理类别、保留 RAR/Manifest/MD5/Word/authority 类别、任务/租约/恢复/冲突摘要、policy/case revision、item digest 和 overall preview digest。
  - 安全与副作用边界：preview 只读取 durable facts，不接受客户端路径/文件清单，不返回路径、表名、owner/token、lease/fence/attempt/context 或敏感原始文件名；不创建 `case_cleanup_runs`、不删除或变更案件工作数据，public route/API/UI 和 Coordinator 仍留给后续任务。
  - 验证：`python -m pytest tests/test_case_retention_preview_service.py tests/test_case_retention_service.py -q --tb=short`：5 passed；`npm.cmd run test:backend`：865 passed、3 skipped、16 warnings；覆盖稳定排序、candidate/skipped/blocked、anchor/due、policy/case revision、任务摘要、digest 稳定性、敏感字段扫描和无 cleanup run 副作用；临时将 `ORDER BY case_id ASC` 改为 `DESC` 后正向 snapshot 测试按预期失败，已恢复并通过。
