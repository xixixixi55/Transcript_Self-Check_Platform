...
export function ArchiveDecisionPanel({
  lifecycle, busy = false, controlsOnly = false, onImmediate, onDeferred,
}: Props) {
  const choices = lifecycle === 'review_ready' ? <Space>
    <Button type="primary" loading={busy} onClick={onImmediate}>立即开始压缩</Button>
    <Button loading={busy} onClick={onDeferred}>稍后压缩</Button>
  </Space> : lifecycle === 'archive_deferred'
    ? <Button loading={busy} onClick={onImmediate}>立即开始压缩</Button>
    : lifecycle === 'archive_interrupted' ? <Space>
        <Button type="primary" loading={busy} onClick={onImmediate}>重新确认并立即压缩</Button>
        <Button loading={busy} onClick={onDeferred}>稍后压缩</Button>
      </Space> : null
  if (controlsOnly) return choices
  if (lifecycle === 'review_ready') return (
    <Alert type="info" showIcon message="报告解析成功，请选择压缩时机。" action={choices} />
  )
  if (lifecycle === 'archive_deferred') return (
    <Alert type="warning" showIcon message="暂未压缩" description="案件和草稿已保留；以后可从案件操作区开始压缩。" action={choices} />
  )
  ...
  if (lifecycle === 'archiving') return (
    <Alert
      ...
      showIcon
      message="正在读取源文件并压缩"
      description="请勿修改、移动或删除源报告目录，也不要继续向该目录写入数据。系统会校验 RAR 完整性、分卷、案件所选文件哈希和清单，但不会为源目录执行额外的重复全量扫描。"
    />
  )
