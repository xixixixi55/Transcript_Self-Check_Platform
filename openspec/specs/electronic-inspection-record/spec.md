...
系统 MUST 满足以下现有合同。工作台编辑通过后端自动保存并携带草稿 revision；编辑会话使用心跳租约，连续无心跳达到既定超时后才允许用户确认接管。版本冲突、租约冲突和保存失败不得静默覆盖后端草稿。系统通过部署实例作用域的后端事实源维护八项笔录默认设置，并在统一平台外壳中提供独立管理入口；案件审核编辑只修改当前案件，不得隐式更新笔录默认设置。
...
- WHEN 用户进入“笔录默认设置”页面
- THEN 系统展示当前委托单位前缀、文号、检查地点、检查方法、检查硬件设备、有序检查人员、光盘编号前缀和文件哈希算法
...
- AND 附件1的“电子数据”和“文件MD5哈希值”数据单元格在首页与续页均允许西文在单词中间换行
...
系统 MUST 为每台取证硬件设备维护名称、型号、所属公司和可选描述。设备列表 API MUST 对缺少公司字段的旧配置返回空字符串兼容值；新建设备必须提供非空所属公司，更新设备时不得因旧客户端未提交公司字段而清除既有公司。

#### Scenario: 查看设备所属公司
- WHEN 民警进入设备管理页面
- THEN 系统展示每台设备的名称、型号、所属公司和描述
- AND 旧配置缺少所属公司时页面显示待补充状态而不是加载失败

#### Scenario: 新增带所属公司的设备
- WHEN 民警填写设备名称、型号和非空所属公司并保存
- THEN 系统持久化全部字段并在刷新后的设备列表中显示
...
- THEN 检查结果、附件1列标题、提取方式、Word 正文和附件3显示所选算法名称与完整大写摘要
...
系统 MUST 在报告解析时按每项检材 IMEI1、IMEI2、序列号是否至少存在一个非空值自动生成“是否可提取”状态，并在审核编辑界面展示且允许人工修正。无法提取时审核页隐藏 IMEI/序列号，Word 检材情况和检查过程均追加“（无法提取）”；缺少该字段的存量数据按同一规则兼容推导。
...
- 每个 `VolumeSlot` MUST 有稳定身份、序号、计划版本和容量/输入范围；标准分卷由用户填写 `GPyyyyMMdd-序号` 首盘号并按实际 part 顺序生成连续光盘编号，超大单卷由用户填写唯一 `YPyyyyMMdd-序号` 硬盘编号；错误介质前缀不得完成映射。
...
内置模板以不可变版本保存。当前 `electronic-inspection-record@1.0.3` 不包含批注结构或附件二示例媒体，保留附件二空白定位段落、VML、分页、表格和动态图片渲染合同，并使用对称 A4 左右页边距、平衡的正文左右排版边界、相对页面居中的附件一固定表格与粗横线、无制表符伪居中的可见主标题，以及一级略突出、同级对齐的结构标题层级。历史 `1.0.0`、`1.0.1`、`1.0.2` 资产继续供既有案件重导出，但不进入新案件可选列表，且不能再设为默认或删除。启动迁移将仍指向历史只读内置版本的默认模板迁移到 `1.0.3`，不改写已有案件引用或用户选择的其他默认模板。
...
#### Scenario: 内置模板卫生升级保持历史案件可复现
- WHEN 部署首次注册清理后的内置模板版本
- THEN 新案件默认引用 `electronic-inspection-record@1.0.3`
- AND 已明确引用 `1.0.0`、`1.0.1` 或 `1.0.2` 的案件继续使用对应历史资产，案件模板引用不变
- AND 新案件不能选择历史内置版本，管理接口不能将其重新设为默认或删除
...
介质编号可在压缩前或压缩后由用户在审核编辑界面输入。标准分卷按 part 顺序生成光盘编号全序列；超大单卷只映射一个硬盘编号。
