...
- 新增 `文枢.exe` 启动入口，负责单实例、后端生命周期、健康检查、浏览器打开、托盘驻留、失败诊断和显式退出清理；正常运行不再保留阻塞式启动窗口。
...
- 启动时继续拒绝清单内必需文件缺失或哈希变化，但清单外普通文件仅写入脱敏告警、不再阻断启动；所有运行时可写配置仍须迁入用户数据根，不得以放宽校验替代程序与数据分离。
...
- 设备配置：`hardware_devices.json` 从冻结程序资源目录迁入默认数据根；用户数据文件不存在时初始化默认设备，并兼容一次性读取旧程序目录中的遗留配置。
