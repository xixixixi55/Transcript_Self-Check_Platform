...
系统 MUST 在报告解析时根据每项检材的 IMEI1、IMEI2、序列号是否至少存在一个非空值生成“是否可提取”状态，并在审核编辑界面展示且允许人工修正。无法提取的检材 MUST 隐藏 IMEI/序列号，检材情况和检查过程 MUST 显示“（无法提取）”。缺少新字段的存量数据 MUST 使用同一标识规则兼容推导。
