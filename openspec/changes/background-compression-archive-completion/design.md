...
**apply 阶段细化（用户实测反馈）**：
- 统一导出交互入口为**案件工作台卡片主按钮**，仅在底层 `archive_complete` 且满足导出条件时将其展示为「待导出」；已导出阶段改以「删除案件」为推荐主操作，并在更多菜单保留「打开案件」「再次导出」。工作台经 `useArchiveCompletionStatuses` 自动加载归档结果，无需先点查看；案件打开页保留「立即/稍后」与补盘号入口，不再承载统一导出触发。
...
- 每个阶段只显示一个实际可见的推荐操作：解析中无主操作，解析失败重试解析，待处理/归档中/待补盘号打开案件，待导出统一导出，已导出删除案件；低频能力收纳到更多菜单。
