...
#### Scenario: Template management keeps only the latest built-in version

- **WHEN** 用户打开笔录模版管理页且注册表同时包含当前与历史内置版本
- **THEN** 管理列表只展示最新内置版本，不展示已退役历史内置版本
- **AND** 启动迁移清理历史内置注册记录并将其案件引用改写为当前版本，不影响自定义模板

### Requirement: 当前测试地区内置模板是唯一分发的内置资产

系统 MUST 只随程序分发当前 `word_templates/template.docx` 测试地区内置模板，不再分发 1.0.0～1.0.2 退役 DOCX 或依赖真实案件内容的一次性模板创建脚本。当前模板 MUST 清除人员、工作站和 WPS 用户标识等不可见元数据；隐私净化不得改变正文、测试地区固定单位文案、页眉页脚、样式、表格、VML、分页、图片区域或最终 Word 的可见业务输出。

#### Scenario: 净化元数据但保持 Word 渲染不变

- **WHEN** 当前测试地区模板执行仓库隐私净化
