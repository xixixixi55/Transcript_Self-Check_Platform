...
#### Scenario: 附件一来源按检材数量展示编号

- **WHEN** 附件1的“来源”包含一至十个有序检材编号
- **THEN** 每个检材编号使用显式换行单独占一行，除最后一个编号外均保留顿号
- **WHEN** 附件1的“来源”包含超过十个有序检材编号
- **THEN** 系统按人工填写的原始文本取第一个和最后一个检材编号，并在同一行展示为“第一个检材编号至最后一个检材编号”；不得解析、补齐或重新生成编号
- **AND** “检材内提取”在编号内容之后单独占一行
