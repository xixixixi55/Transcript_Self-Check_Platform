...
#### Scenario: 附件1按四条分卷上限优先填满前页并承载签名区
- WHEN 最终 `ArchiveManifest` 包含一条或多条实际分卷
- THEN 附件1按顺序每页最多放置四条分卷数据并优先填满前页，最后一个数据页在一至四条分卷后直接附带固定手写签名区
- AND 四条分卷生成单个“4 条分卷 + 签名区”页面，五条和九条分卷分别按 `[4,1]`、`[4,4,1]` 分页，不得为均衡末页而改为 `[3,2]` 或 `[4,3,2]`
