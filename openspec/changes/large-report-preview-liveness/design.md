...
2026-09-10 的外部只读诊断确认：New 报告导航中的设备节点可能使用“总数/有效数”双计数标签。导航绑定失败会使单检材报告退回逐个探测同一 Base 目录的大量 `data_` JSON。候选发现应同时接受单计数和双计数标签，并继续以 `filePath`、`varName` 和检材目录的唯一对应关系作为绑定条件；不得放宽为任意名称匹配。

持久化解析缓存移除后，`CandidateDirectoryIndex` 不再有生产消费者。请求快照只保留实际读取依赖的记录和摘要，不再为所有未选候选执行逐文件 `stat`、排序或保存内部索引。安全链接检查仍由目录枚举和实际依赖读取边界承担。

工作台解析任务采用约一秒的短轮询，以减少后端已完成而页面仍显示“解析中”的等待；长时间归档任务继续使用原有较低频率。两类轮询使用独立计时器，并由已有请求互斥避免重叠请求。
...
| 20 | 快照、候选选择、实际依赖元数据/摘要读取 | 不组装响应或编排服务 |
