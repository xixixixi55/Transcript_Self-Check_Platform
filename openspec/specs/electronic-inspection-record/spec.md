...
- **AND** 上方历史处理区按办理顺序展示已经完成的用户操作、报告自动识别与系统自动沿用的信息摘要、后台任务阶段变化和已经恢复的异常，不重复展开完整审核表单，也不改造成逐句聊天记录或消息气泡
- **AND** 只有下方当前对话区采用紧凑 Chat 形式展示獬豸助手身份、可替换的角色占位、当前推荐事项、结构化控件和主要操作；没有用户事项时显示等待后台处理、办理完成或现有恢复入口
- **AND** 每个当前事项先由獬豸助手使用完整、可执行的提示句说明下一步，例如“请输入文号”“请选择委托时间”或“请恢复草稿保存”，不得只显示“处理文号”等状态标签后直接呈现控件
- **AND** 当前事项在本次页面会话中完成后，下方对话区可以短暂展示“用户已完成该项”的上一轮回复及助手承接语，再显示下一条提示；刷新或重新进入不承诺还原该回执，不保存逐字输入或新增持久化聊天记录
- **AND** 当前对话区按实际内容决定高度并尽量少占空间，使历史处理区保持为主要浏览区域；未来替换角色形象不得改变文字和控件的可用性
