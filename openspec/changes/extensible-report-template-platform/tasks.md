...
- [x] 12.8 将当前测试地区 `template.docx` 收敛为唯一内置模板资产：删除 1.0.0～1.0.2 退役 DOCX 和三个一次性创建脚本；净化当前模板的核心属性、自定义属性和 WPS `docVars`，但逐字节保留所有渲染相关 OOXML 部件；发布新的当前内置版本并将旧内置默认值及案件引用幂等迁移到新版本。文件：`word_templates/`、`scripts/clean_template_docx.py`、`scripts/check-repository-assets.ts`、TemplateProfile/WorkbenchFactory/模板引用 Repository、资产文档与 living spec。
