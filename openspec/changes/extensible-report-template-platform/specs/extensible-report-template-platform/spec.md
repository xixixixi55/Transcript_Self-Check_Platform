...
阶段一 MUST 只允许最终检材类型 `phone`/`tablet`，允许报告明确且无冲突的类型自动预选；缺少可靠类型时，只有 IMEI1 与 IMEI2 均为有效、不同的 15 位数字才可兜底推断 `phone`，以报告确认状态保留推断诊断并直接通过检材类型导出门控。单个、非法或重复 IMEI 不得推断，明确平板或类型冲突不得被该兜底覆盖，其余情况由审核页面确认。主取证软件无法可靠识别时允许审核页面分别编辑名称和版本，但确认前不得正式导出。WinRAR 缺失或不可调用时允许上传、解析和编辑，禁止自动压缩、禁止最终正式导出、不生成 `ArchiveManifest`，且不降级 ZIP。0 张图片不生成附件二页面，正偶数图片按页面计划生成，奇数图片阻止导出；附件二缺失不重排附件三编号。人员快照按用户选择顺序一人一行；附件一每页最多三条分卷并从前向后优先填满，最后一个数据页少于三卷时在最后一卷后生成“以下空白”和所需普通空白补位行以补足三个分卷槽位，恰好三卷时不生成第四个补位行，再于页面底部承载不可拆分的人员框，不得生成签名独立页。SHA-256 的64位摘要保持16磅字号、移除模板空段落并以无重叠字符宽度适配，分页规则与 MD5 一致。`shadow` 不调用 WinRAR、不执行真实重复压缩、不产生第二份正式文书。
