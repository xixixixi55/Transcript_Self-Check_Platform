...
**决策**：后台任务在每 part 通过完整性/MD5 校验时，回调「附件回填服务」，将该 part 的文件名/大小/MD5 写入案件记录（检查结果 `result` 字段 + 附件1 `extract_list` 行），覆盖既有值（含手工编辑）。

**理由**：需求明确「每个最终 RAR 完成后同步填写检查结果与附件1」且已确认「实时填且覆盖手工值」；按 part 增量回填避免全部完成后一次性写。

**备选与拒绝**：
- 全部完成后一次性填（现状 usePreviewArchive）→ 与「每 RAR 完成实时同步」相悖，拒绝。
